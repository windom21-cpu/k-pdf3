...
- **(未リリース、2026-07-23 main 反映済 `97c7243`+`e86f5ef`)** = **S6 リファクタ (REVIEW-2026-07 #8) 着手 — その1/その2 完了**。renderer.js 8,050 → 7,405 行。**(その1)** `image-export.js` 新設 (β97 画像書き出し 2 機能: `actionExportAsImage`/`startRegionImageDrag`/`saveRegionImage`/`showImageExportDialog`、~390 行) **(その2)** `save-flow.js` 新設 (確定/上書き/別名の共通パイプライン + ADR-0026 戻す: `actionSave`/`actionExportToPath`/`actionRestoreEditableMaster`/`refreshRestoreMasterUI`/`isPdfOutOfSync`/`refreshDocStateField`、~420 行)。**いずれも純移動・機能変更ゼロ** (§4.4 パターン 1 の getter 注入。`workspaceMutated` 書込は setter 注入、menuBar は `setMenuEnabled` callback — early boot の TDZ throw は従来どおり catch)。各 commit で npm test 全緑 (11 スイート fail 0)。タグ無し = CI ビルド未発火・業務実機は v2.0.20 のまま。**残り: その3 = sidebar-thumbs.js (~1,300 行、最大の塊) → その4 = split-view.js (~600 行) → (余力) ページ番号一括/フォーム Tab 順**。回転系 (`rotatePageBy` 周辺) は完成領域のため**動かさない**。完了条件 = npm test + 実機一巡 (開く→編集→保存→書き出し→印刷→タブ tearout/dock-back、全抽出後にまとめて実施予定)。**着手動機 = 下記テキスト表示ずれ (§8.2 🆕) の根治 (案 C) を「次の大物」と位置づけたため**。memory [[project_s6_refactor_progress]]
...
renderer.js は中央 orchestrator として残し、機能別モジュールに分散 (B2 完了時 4,472 行 → 2026-07-10 実測 8,019 行に再肥大 → **S6 リファクタ = REVIEW-2026-07 #8 を 2026-07-23 に着手、その1/その2 抽出後 7,405 行**。残り = sidebar-thumbs / split-view 抽出、冒頭の未リリース欄参照):
...
renderer.js に残る責務: `isOpen` / `projectStore` / `history` / `viewer` 等の core state、`refreshViewer` / `actionOpen` / `openPdfPath` / `actionExport` (別名ダイアログ→save-flow 委譲)、split-view、sidebar thumbs、search、各種メニュー glue。**S6 残り: sidebar-thumbs + split-view 抽出が進行中** (冒頭の未リリース欄参照)。
...
- **S6 (split-view + sidebar-thumbs 抽出)**: B2 残置責務。**2026-07-23 に着手 (REVIEW #8)** — その1 image-export / その2 save-flow 抽出済、sidebar-thumbs / split-view が残り (冒頭の未リリース欄参照)
