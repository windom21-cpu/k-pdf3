...
最終更新: 2026-05-14（β70 配布中 / Actions storage budget fix / **B2 リファクタ S1+S2 着地** / **印刷経路 Adobe Reader CLI 採用で一旦区切り**） / 現在のバージョン: **v2.0.0-beta.70（β37-β41 で日付スタンプ年月のみバリアント + プレビュー / ゴースト統一 + 印刷 overstroke 調整、β42-β43 で FAX 経路対応、β44 でウインドウサイズ永続化、β45 で削除ダイアログ視覚位置 + サムネ右クリック削除、β46-β50 で印刷プロパティ完全反映 + DEVMODE 同期 restore + 印刷中クローズ確認、β51 で診断クラッシュロガー、β52-β53 で FAX 誤検出修正 + byte-copy も Sumatra 経路統一による Apeos C2360 ハング解消、β54 で splitThumbSelection クリア + FAX 規定プリンタ切替、β55-β63 で印刷品質改善試行 (PrintAsImage / 案 M GDI / 案 N PostScript / 案 N' PCL / ζ Phase 1 font embed) が全て C2360 ドライバの構造的制約に阻まれ全面撤回、β64-β70 で C アプローチ = PDF Reader CLI 委譲経路を採用、β60 で DEVMODE 永続化、β61 で FAX 送信先リセット、β62 で overlay PNG bbox 縮小、β70 で印刷エンジン選択 UI + Acrobat Pro ウィンドウ強制 hide）** / **2026-05-14 内に: GitHub Actions storage を 0.5GB 上限内に収める CI 改修 + B2 リファクタ S1+S2 完了 (renderer.js 8631→7297、-15.5%、6 ファイルに分散) + 印刷経路 Adobe Reader CLI 委譲で一旦完了**
...
> - **β36 (G1〜G2 吹き出し UX)**: テスター指摘「吹き出しの文字が枠の下付きになる、上下の余白が不均等」「テキストと同様に確定前 wrap がそのまま残ってほしい」に対応:
>   - **G1 吹き出し枠ぴったり化**: `CALLOUT_LINE_HEIGHT = 1.0` (元 1.2)、`CALLOUT_PAD_Y_TOP = CALLOUT_PAD_Y_BOTTOM = 1pt` (border のみ、leading slack ゼロ)。line-height 1.2 では CJK font metric の leading 配分で line-box 内の glyph が下寄りに描画され、border との隙間が下に偏って見えていた。line-height 1.0 で line-box = font-size ぴったり、glyph が枠と密接。
>   - **G2 確定前 wrap 維持 + commit path 統一**: `handleTextEditCommit` を text overlay と callout 共通 path に変更。viewer の `enterTextEdit/finish` で **editing 中の `el.offsetWidth/Height` をスナップショット → canonical 値で renderer へ渡す**。renderer は visible 値があれば優先、無ければ measure 関数で fallback。measureCalloutSize の signature を text overlay と並列に揃え、`(text, fontSize, fontFamily, currentW, maxW)` で char-wrap を内蔵。これで text overlay の β25 C1 と callout の挙動が完全同一 recipe に統一。
>   - **wrap area 一致のサブ修正**: editing 中の `el` (contentEditable) に textNode と同じ `padding: 0 4px` + `line-height: 1.0` を inline set。editing 中の wrap area = 確定後 textNode 内の wrap area = `outer - 10pt`。これを揃えないと visible H で box を保存しても確定後 wrap で 1-2 行余分に折れて下に空白が残る (β36 開発中に途中段階で発覚 → 修正済)。
>   - **exporter**: `padY = 1 * zoom` (CALLOUT_PAD_Y_TOP/BOTTOM と同期)、`lineHeight = fontSize * (props.lineHeight ?? 1)` は元から 1.0 で整合済。
>   - 既存 callout は properties.h が保存値のままなので変化なし。新規 callout / リサイズ / 再編集で h が再計算されて新 layout に。
> - **β37 (H1)**: 日付スタンプに「年月のみ」字間調整バリアント `date-numeric-spaced-2` を追加。押印日の和暦年・月だけ自動展開 (今日なら "-8 -5"、来月なら "-8 -6"、来年なら "-9 -5")、日部分は空欄。`distribute-2` spacingMode を新設 (既存 distribute-3 と同じ `justify-content: space-between` ロジック)、viewer/exporter の条件に `|| "distribute-2"` 追加するだけで対応。
> - **β38 (H2)**: 日付スタンプ登録 dialog のプレビューが distribute モード非対応で「-8 -5」が中央 1 行表示 → 実押印 (左右端) と乖離。paintStampPreview に spacingMode を渡し distribute 時は token を box 左右端配置。
> - **β39 (I1/I2)**: タブ切替時の水平スクロールずれ + しおり右クリック→名前を変更が効かない 2 件。I1 は applyTab の RAF で `scrollTop` だけ復元し `scrollLeft` が前タブから持ち越されていた → `scrollLeft = 0` リセット。I2 は ctxBookmark の pointerdown が `preventDefault` を呼ばず、mousedown のネイティブ focus shift で `input.focus()` 直後に blur → finish(true) が値未変更で no-op していた。preventDefault 追加 + rAF defer で二重防御。
> - **β40 (I3)**: 配置時のカーソル追従ゴーストが distribute 時に縦積み表示。ゴーストの box 幅は stamp font の measureText で決定するが、`.stamp-ghost` CSS は body 既定フォント継承で text が overflow → 半角スペース位置で折返し → 縦積み。viewer/exporter と同じく `display: flex; justify-content: space-between` + token span 分割 + `white-space: nowrap` で folding 抑止。
> - **β41 (I4)**: 日付スタンプの印刷時 overstroke を撤回。β31 D1 で text/callout/stamp の glyph 描画を paintGlyphRun に統一する際 stamp 全部「常時 bold (`lineWidth = fontSize * 0.06`)」になっていた。β34 で text は 太字 OFF=0.03 に分離したが stamp は印影 (text stamp) 想定で default bold のまま。日付印は印影ではないので pre-β31 baseline (`stroke: false` で素 fillText) に戻す。実装は `paintGlyphRun(opts.stroke = false)` + `currentStampPreset` / `placeStamp` で `stampKind` ("date" / "text" / "image") を新規 placement に保存、exporter で stampKind === "date" を判定。既存 placement (stampKind 未設定) は `spacingMode === "distribute-*"` 経路 (drawSpacedTokensOnCanvas) で自動カバー、非 distribute の古い日付スタンプは bold のまま (再配置で更新)。
> - **β42-β43 (J1/J2)**: 編集済 PDF を複合機 FAX キューに送信したいユーザー要望。
>   - **β42 J1**: SumatraPDF を `-silent` 付きで起動すると FAX ドライバの送信先入力ダイアログが抑止されて exit 1 で失敗 (stderr も silent で消える)。`isFaxDevice()` を導入し fax 系名 (`/fax/i` + カタカナ「ファックス」「ファクス」「ﾌｧｯｸｽ」「ﾌｧｸｽ」) は -silent を外す。Chromium 経路 (silentPrintPdf) も同様に `silent: false` に切り替え。renderer の待機メッセージで「ドライバの送信先入力ダイアログをご確認ください」を予告表示。
>   - **β43 J2**: β42 でも「プリンタを初期化できませんでした」+ exit 1。Sumatra の mupdf+WinSpool は FAX キューの DEVMODE 初期化に対応していない (通常プリンタ向けの構造依存)。FAX 時は rasterized でも Sumatra をバイパスして silentPrintPdf に流す (β42 で silent:false 化されているので OS 印刷ダイアログ経由 → 送信先入力 → 送信)。
> - **β44 (K1)**: メインウインドウのサイズ・位置を永続化。`userData/window-state.json` に bounds + maximized を保存、close イベントで 1 回だけ書き込み (resize/move 連打回避)、起動時に復元。off-screen ガード (外部モニタ切断後にウインドウが見えなくなる事故防止) + 最大化状態は `getNormalBounds()` で un-maximize 時のサイズを記憶。
> - **β45 (L1/L2)**: テスター要望 2 件。
>   - **L1**: 削除確認ダイアログのページ番号が `p.3, p.5` のように元 PDF の pageNo をそのまま出していて、挿入ページがあると番号が飛んで「サイドバーで見ている位置と一致しない」指摘。`viewer.registry.posOfPageNo()` で 1-indexed 視覚位置に変換し「3 ページ目 (元)」「5 ページ目 (挿入)」形式に統一。
>   - **L2**: サムネ右クリックメニューに「ページを削除」追加。multi-select 時は save-page と同じく選択全体削除 (oneShot state で deleteSelectedPages を起動して確認ダイアログ + pending/synthetic 分岐の既存ロジック再利用)。
> - **β46-β50 (J3〜J6)**: 印刷プロパティの設定が反映されないユーザー報告から始まる連鎖。
>   - **β46 J3**: DocumentPropertiesW IDOK 戻り DEVMODE から β17 では copies / orientation だけ抽出 + 他は破棄していたので、両面 / トレイ / 白黒のプロパティ設定が Sumatra に伝わっていなかった。printer-properties-win.js で `dmDuplex` (offset 94) / `dmDefaultSource` (88) / `dmColor` (92) を抽出、Sumatra の `-print-settings` に simplex/duplex/duplexshort + color/monochrome + bin=N を追加、Chromium 経路の `duplexMode` + `color` にも反映。
>   - **β47 J4 + J5**: 両面は反映されたがトレイだけ反映されない + アップデート後 zombie プロセス問題。**J4** は β46 の flag gate (`DM_DEFAULTSOURCE_FLAG`) が厳しすぎた件 — 複合機 driver は Windows コントロールパネルでトレイを persistent default に設定していると本アプリのダイアログでの「変更操作」がない → flag を立てない → 値は正しいのに skip されていた。flag gate を撤廃し値の妥当性 (bin > 0 / 既知 enum) で判定。**J5** は cold start (PDF arg なし) で `requestSingleInstanceLock` 失敗時に `taskkill /F /IM K-PDF3.exe /FI "PID ne self"` で zombie 掃除 + 最大 1 秒 lock 再試行。
>   - **β48 J4b + J5b**: トレイがまだ反映されない件 → 複合機 driver はプリセット保存時にトレイ情報を `dmDriverExtra` (driver-private DEVMODE 拡張) に格納する設計 (FUJIFILM Apeos 等)。Sumatra の `bin=N` は公開 `dmDefaultSource` しか触れない。Adobe Reader 採用方式の **SetPrinter level 9 (PRINTER_INFO_9)** で印刷直前に per-user DEVMODE 既定をユーザー修正済 DEVMODE (driver-private bytes 含む) に差し替え、Sumatra/Chromium にそこから拾わせる。printer-properties-win.js に koffi `GetPrinterW` + `SetPrinterW` 追加、IDOK 時 DEVMODE full buffer を Map<deviceName, Buffer> にキャッシュ、`applyUserPrinterDevmode` + `restoreUserPrinterDevmode` export。J5b は β47 の `Atomics.wait + SharedArrayBuffer` が Electron main で TypeError を投げる (shared memory が default 無効) → unhandled error で新版起動がサイレントクラッシュ。busy-wait + try/catch でくるんで防御。
>   - **β49 J4c**: 印刷ジョブ実行中にアプリ閉じ → `process.exit()` が in-flight Promise の `finally` を待たず即終了 → per-user DEVMODE が修正状態のまま残留する corner case。`_inflightDevmodeToken` を printer-properties-win.js の module 変数で保持、`restoreInflightDevmodeSync()` を export (`_native` ロード済前提で同期 SetPrinter)、main.js の既存 `before-quit` hook 冒頭で呼ぶ。
>   - **β50 J6**: ユーザー提案。印刷ジョブ進行中にメインウインドウ閉じる → `mainWindow.close` を intercept、`_printInFlight` (`print-pdf-silent` IPC で set/clear) が立っていれば showMessageBoxSync で「完了まで待つ (default) / 印刷をキャンセルして終了」。後者は `cancelInFlightPrint()` で sumatra kill + printWindow destroy → 通常クローズ → β49 の sync DEVMODE restore で per-user 既定も復元。
> - **β51 J7**: クラッシュ診断ロガー。`userData/crash.log` に `process.on('uncaughtException' / 'unhandledRejection')` / `app.on('render-process-gone' / 'child-process-gone')` を timestamp 付き append。`session-start` は `whenReady` で記録 (`app.getPath` を安全に呼ぶため)。新 IPC `kpdf3:open-crash-log` (shell.openPath で OS 関連付け = Win なら Notepad) + ヘルプメニュー「クラッシュログを開く」+ preload `openCrashLog` ブリッジ。**stable リリース時に撤去予定** (β 期間限定の仮設、ユーザー指示あり)。
> - **β52-β53 (J3b/J7b/J8)**: ユーザー報告「印刷ができなくなった: silent print failed」「送信中でフリーズ」。
>   - **β52 J3b**: isFaxDevice の `/fax/i` substring が緩すぎた仮説で語境界マッチに変更 (`/(?:^|[\s_\-()/\\:])fax(?:$|[\s_\-()/\\:])/i`)、"Faxable" や "FAX対応" の中の fax は誤検出しない。**J7b**: print-pdf-silent IPC で経路選択結果 (deviceName / source / isFax / useSumatra / sumatraExe / 全 opts) を crash.log に `print-route` で記録、silentPrintPdf 失敗時の errorType + printOpts も `silent-print-failed` で記録。
>   - **β53 J8**: β52 の print-route ログから真因判明 → `deviceName=FUJIFILM Apeos C2360 / source=byte-copy / useSumatra=false`。HANDOVER §β3 記録の「Apeos C2360 + Chromium silent print ~55s ハング」が byte-copy 経路で生き残っていた (β3 当時 rasterized だけ Sumatra に逃がして byte-copy は「ソース PDF normal-shape だから Chromium で OK」と残置)。`useSumatra` 判定から `source === "rasterized"` 制約を撤廃 → Win + 非 FAX + Sumatra バイナリあり → どの source でも Sumatra に流す。Chromium silent を踏むのは FAX (silent:false で OS dialog 経由) だけに限定。実機で印刷成功確認済。
> - **β54 (2026-05-14、印刷品質改善着工 + UX 修正 2 件)**:
>   - 分割画面で選択 → 解除しても `splitThumbSelection` が残って次の印刷時に偽の preselect になる事象を修正。`setSplitMode(false)` 時に selection をクリア。
>   - FAX 印刷の「OS ダイアログが規定プリンタを表示してまた FAX 選択させられる煩」を修正。Win32 `Get/SetDefaultPrinterW` で印刷直前に FAX を一時規定にし、callback / cancel / before-quit で復元する `applyFaxAsDefaultPrinter` / `restoreDefaultPrinter` / `restoreInflightDefaultPrinterSync` を `printer-properties-win.js` に追加。
> - **β55-β63 (印刷品質改善の試行錯誤、全て構造的制約に阻まれ撤回)**:
>   - β55 で「DL PDF が荒い、明朝が著しい」ユーザ報告 → Sumatra 3.6.1 の portable settings `PrinterDefaults.PrintAsImage = true` を試みるも、Sumatra 3.6.1 のバイナリに該当キー自体が存在しないことが β56 strings 解析で判明 → 撤回
>   - β56 で **案 M (Win32 GDI 直接ラスタ)** を実装。mupdf でプリンタ DPI のビットマップを焼いて StretchDIBits 送信。ユーザ実機検証で「Sumatra と同等の荒さ」と判明 (raster 経路の天井に到達、vector 経路でないと Adobe 同等にならない)。β57 で撤去
>   - β57 で **案 N (PostScript raw print)** を実装。mupdf `DocumentWriter(buf, "ps", "")` で PS bytes 化 + Win32 `WritePrinter` RAW で直送。C2360 でエラー 106-726 (PDL データ不正系)
>   - β58 で setpagedevice prelude を撤去 + **PCL writer 経由のカスケード** を試行。C2360 でエラー 016-726 (プリント言語自動判定エラー)。RAW datatype 経路はドライバを完全バイパスするので C2360 の auto-detect で識別されない構造を確定
>   - β59 で β56-β58 を全面 revert、Sumatra 第一選択の β55 水準に戻す
>   - **γ アプローチ**: ユーザがプロパティで C2360 ドライバ「**細い線を太く**」を ON にすると明朝が Adobe 同等まで改善。線幅補正は driver 機能で K-PDF3 側コード不要、β48 J4b の SetPrinter level 9 経由で per-user 既定に保存される
>   - β60 で **F1**: プロパティを開いたとき DocumentPropertiesW の `pDevModeInput` に cached DEVMODE を渡して driver UI が「ユーザの前回設定」を起点に開くようにする (DM_IN_BUFFER フラグ併用)。**F2**: `_userDevmodeCache` を `userData/printer-devmode-cache.json` に base64 で persist + 起動時 rehydrate → 「細い線を太く」がアプリ再起動後も継承
>   - β61 で **FAX 送信先リセット**: `applyCleanFaxDevmode` を新設し、FAX 経路では driver-private DEVMODE bytes (dmDriverExtra) を 0 埋めしてから SetPrinter level 9 push。FUJIFILM Apeos C2360 FAX 等が driver-private に「最後の宛先」を残す挙動への対策で、毎送信前に宛先欄を空でリセット
>   - β62 で **overlay PNG を bbox 縮小**: スタンプ付き PDF 印刷で全面 raster fallback が起きていた件 → `composeOverlayOnlyPage` の戻り値を `{canvas, bboxPt}` に拡張、overlays の union 矩形 + 8pt PAD で canvas を最小化、`assembleHybridPdf` の `drawImage` も bbox 位置・サイズで配置。本文 vector の範囲が広がる
>   - β62 patch: 吹き出し矢印先端 (`arrowDx/arrowDy`、default 値が box 外を指す) が bbox から漏れて canvas クリップされていた regression を ARROWHEAD_PAD=14pt の追加で修正
>   - β63 で **ζ Phase 1 (vector text overlay)** を試行。`@pdf-lib/fontkit@1.1.1` 追加、Yu Mincho TTF を runtime read + embedFont、pdf-lib drawText で text overlay (rot=0) を vector 描画。ユーザ実機検証で「テキスト overlay があるだけで元 PDF も荒くなる」と判明 → **C2360 ドライバは embedded CID TrueType フォントの存在もトリガに全面 raster fallback** することが判明。font-loader-win.js / overlay-pdf-draw.js / @pdf-lib/fontkit すべて β64 で revert
> - **β64-β70 (C アプローチ採用、印刷経路を Adobe/Foxit/PDF-XChange CLI 委譲に切替えて一旦区切り)**:
>   - **β64**: β63 全 revert + 三段カスケード経路再設計。`pdf-reader-finder.js` で Adobe Acrobat Reader DC / Adobe Acrobat Pro / Foxit / PDF-XChange を auto-detect、`printPdfViaPdfReader` で CLI 起動 (`AcroRd32.exe /n /t pdf printer` 等)。Sumatra は fallback として温存、最終手段で Chromium silent。Pro 限定環境 (Reader DC 排他のため) でも Adobe 経由経路が確立
>   - **β65**: Adobe Reader 2 重印刷 + ウィンドウ残留を修正。`/n /t` だけだった引数を `/n /s /o /h /t` に拡張 (splash / open dialog / hidden 起動の 3 フラグ追加)。exit code 非ゼロでも success 扱いに変更 (Adobe Reader DC が成功時に code=1 返す quirk 対応、`reject → catch → Sumatra fallback` で 2 重印刷していた問題)。Reader DC を Acrobat Pro より優先する検出順位に変更
>   - **β66**: 印刷起因の新規 Acrobat.exe を `taskkill /F /T` で除去するクリーンアップ機構。`getProcessPidsByName` + `killNewPdfReaderProcesses` で before/after PID 差分を取り、ユーザの既存 Adobe ウィンドウは保護
>   - **β67**: kill 対象に Acrobat ヘルパー (AcroCEF.exe / AcroBroker.exe / AcroFlattener.exe) を追加。`PDF_READER_HELPER_EXES` テーブルで engine 別にヘルパー名を保持
>   - **β68**: Acrobat Pro の `/h` フラグ撤去 + 120 秒安全網タイムアウトを試行 (Reader DC は `/h` honor、Pro は editor 設計で `/h` 受けても自然 exit しない quirk の対策仮説)。ユーザ実機検証で **Pro は `/h` 有無に関係なく自然 exit しない** (β64 ログの「32 秒 exit」もユーザ手動と判明)
>   - **β69**: 戦略改訂。`/h` は全 engine で維持、SPAWN_TIMEOUT_MS を engine 別に分岐 (Acrobat Pro=30 秒、それ以外 = 120 秒)。Pro は自然 exit しない仕様を受け入れ、30 秒タイマで強制 kill する主要 exit メカニズム化。print spool 投入には 30 秒で十分なので、強制 kill で印刷影響なし
>   - **β70**: 印刷エンジン選択 UI (約束履行) + Acrobat Pro ウィンドウ強制 hide。
>     - `findAllPdfReaders()` で全候補列挙、新 IPC `kpdf3:list-print-engines` で renderer に返却。印刷ダイアログのプリンタ fieldset に「経由:」select を追加、`localStorage["kpdf3.printEngine"]` で永続化。`printPdfSilent` payload に `engineOverride` を載せて経路選択を上書き
>     - PowerShell + `Add-Type` で `user32.dll ShowWindowAsync(SW_HIDE)` を 5 秒間 200ms 毎に polling 呼び出し、protect PID で既存ウィンドウを保護しつつ印刷起因の新規 Adobe ウィンドウを hide。Pro の `/h` 未対応 quirk への代替策
> 
> **Adobe DC 時代の Pro/Reader 排他制約 (β70 で判明)**: Acrobat DC (2015〜) では Pro と Reader が同じ ProductCode `{AC76BA86-1033-FF00-7760-BC15014EA700}` を共有しており、**併存インストール不可**。winget で Reader を追加しようとすると Pro への MSP patch 適用扱いになりエラー 1603。Acrobat 11 まではこの制約は無かった (世代別 ProductCode)。Pro 環境のユーザは Reader 経由経路に切替不可、β70 の Pro 用 30 秒タイマ + ウィンドウ hide で対応する想定。
> 
...
> 
> **β70 時点での未解決テスター要望 / 残課題**:
> - **印刷経路の残存課題 (構造的制約、回避不可)**:
>   - **Acrobat Pro 環境では 30 秒の体感待ち**: Pro DC は CLI `/n /t /h` でも自然 exit しない editor 設計のため、β69 の 30 秒強制 kill タイマが主要 exit メカニズム。Reader DC は排他 (Adobe DC ProductCode 共有) で代替不可。受容。
>   - **画像スタンプは bbox raster** (β62): 画像はそもそも raster なので image XObject は不可避。bbox 縮小で影響範囲は最小化済 (8pt PAD のみ)。
>   - **β55 で書いた `SumatraPDF-settings.txt` の残骸**: 既存ユーザ環境の `%resources%\sumatrapdf\` に PrintAsImage=true の記述が残っている可能性。Sumatra 3.6.1 は不明キーを無視するので実害なし。次回 K-PDF3 再インストールで消える。放置で OK。
> - **C3 Adobe で押したスタンプ (annotation) が viewer で見えない（印刷では出る）**: §15.3 の annotation read-only proxy 該当、新セッション規模。実害は viewer 表示のみで印刷物には出る。
> - **「後で」仮説の検証と恒久対応**: 上記の autoUpdater 経路改修。再現できれば次の β で対応。
> - **β36 吹き出し line-height 1.0 トレードオフ**: 文字密接化のため line-height を 1.2→1.0 に下げたので、複数行吹き出しの **行間が詰まる** 副作用がある。読みづらいなら 1.1 等の中間値 or 上下対称化の別アプローチを再検討。実機での読みやすさユーザー判断待ち。
...
> - **PDF 開いて閉じて、次の PDF 開こうとするとクラッシュ事象** (β51 ユーザー報告): コードレビューでは致命的な lifecycle 不整合は見つからず、mupdf-js WASM heap 蓄積 / IPC レース / SQLite ロック / 非アクティブタブの inserted-source cache 漏れ等の複数仮説あり。β51 でクラッシュ診断ロガーを仕込んだので次に再現したら crash.log で要因特定可能。
> - **「新しい PDF を別ウインドウで開く」要望** (2026-05-13 ユーザー): 個別実装は見送り、**タブ切出し (Chrome 風ドラッグアウト) 機能を実装する段階で合算実装**することで合意。共通インフラ (main の state を per-window 化 / 複数 BrowserWindow 生成 / タブ workspace IPC 移送) がほぼ同じなので別々にやると無駄になる。「別ウインドウで開く」= 「新ウインドウ + 1 タブ」はタブ切出しの strict subset。
> - **stable リリース時にやること**:
>   - β51 で追加した **クラッシュ診断ロガーを撤去** (β 期間限定の仮設、ユーザー指示あり)。撤去対象: `crashLogPath()` / `logCrash()` 関数 + 全イベントリスナ + `session-start` ログ + `print-route` / `print-route-end` / `print-pdfreader-failed` / `pdfreader-cleanup` / `pdfreader-timeout` / `silent-print-failed` ログ呼び出し + `kpdf3:open-crash-log` IPC + preload `openCrashLog` ブリッジ + index.html の `data-action="open-crash-log"` menu-item + renderer の `actionOpenCrashLog` + menu dispatcher entry + status hint entry。他で使ってなければ `appendFileSync` import も。
> 
> **印刷経路は β70 で一旦区切り**。引き続きの主軸は B2 リファクタ (renderer.js モジュール分離 S3-S5)、タブ切出し D&D (フェーズ 2)、annotation read-only proxy、qpdf sanitize 等。印刷経路で何か新たなユーザ報告が出たら、§β70 の三段カスケード設計 (PDF Reader CLI → Sumatra → Chromium silent) を起点に再開する。
> 
...
git log --oneline | head -30       # 最新は β36 (吹き出し枠ぴったり化 + commit path 統一) のはず
...
**S4 (print-flow) 着手時の注意 (β70 時点)**:

β54-β70 で renderer.js の print-flow に **以下の追記** が入っているので、S4 切出し時に
維持すること:

- **β54**: `setSplitMode(false)` 内で `splitThumbSelection.pageNos.clear()` + `splitThumbSelection.anchor = null` + `refreshThumbSelectionVisuals()` を呼ぶ (分割画面で選択 → 解除しても残って次の印刷時に偽 preselect になる事象の修正)
- **β70**: 印刷ダイアログ表示時 `showPrintDialog` 入口で `ensurePrintEnginesPopulated()` を呼ぶ。`printEngineSelect = $("print-engine")` + 1 回だけ `kpdf3.listPrintEngines()` で populate して `localStorage["kpdf3.printEngine"]` で永続化 + `change` event で保存。`actionPrint` 内の `kpdf3.printPdfSilent({...})` 呼び出しに `engineOverride: printEngineSelect?.value || null` を追加。
- **β62 / β62 patch**: `composeOverlayOnlyPage` の戻り値が `{canvas, bboxPt}` に変更されているので caller (composePagesForExport) で `const { canvas, bboxPt: bb } = await composeOverlayOnlyPage(row, projectStore.getPageOverlays(row.pageNo), EXPORT_ZOOM);` の形に。bboxPt は output entry の `overlayBBox` に詰める。callout 矢印先端の bbox 算入は `composeOverlayOnlyPage` 内で完結。
- **既存 (β50 J6)**: `kpdf3.cancelPrint?.()` を onCancel に渡す busy modal。`_printInFlight` トラッキングは main 側完結。

これらは print 関連 (`showPrintDialog` / `actionPrint` / busy modal / 印刷経路全般) なので **S4 で切り出す予定の領域**。S4 で `print-flow.js` (仮称) に移す時は上記をすべて持っていく。
