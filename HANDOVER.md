...
最終更新: 2026-07-06
現在のバージョン: **v2.0.13-beta.1 (2026-07-06 リリース、β トライアル配信中)**。直前の stable は **v2.0.11 (2026-06-30)**。β.1〜β.150 の業務並走を経て 2026-06-05 に v2.0.0 stable へ卒業 (Win+Mac+Linux 3 OS 配布)。**ADR-0026「戻せる確定保存」(v2.0.12-beta.1) と ADR-0027「ワークスペースの整理」(beta.2) に加え、v2.0.13-beta.1 で「MS 明朝が印刷で薄い」のベクターテキスト埋め込みによる構造解決を投入し、`releaseType=prerelease` のまま β トライアル継続中** (ADR-0027 は実機確認済み、ADR-0026 は業務での往復動作を運用確認、v2.0.12-beta.3=パスワード平文化警告 / beta.4=byte-copy ゲート一本化は実運用の中で確認する扱い → 不具合なければ stable v2.0.13 へ昇格)。**⚠️ stable へ昇格する時は `package.json` `build.publish.releaseType` を `prerelease`→`release`、`version` を `2.0.13-beta.x`→`2.0.13` に戻す** (β タグ `v*-beta.*` は CI で Windows のみビルド、stable タグは 3 OS)。**基本は stable 運用** (重大バグは patch 2.0.x、新機能/大物は ADR 起草後 → β トライアル → 昇格)。**Mac/Linux を Windows 同等 (印刷/FAX/Office/署名) にするには追加開発が要る (§15.6)**。
...
#### 🟡 MS 明朝を「フォント一覧から選択」するとベクター濃度化 (v2.0.13) が効かず薄いまま — 2026-07-06 (ユーザーは MS 明朝を多用するので重要)
