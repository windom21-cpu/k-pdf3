...
- **(未リリース、2026-07-23 main 反映済 `97c7243`+`e86f5ef`+`ee3e03f`)** = **S6 リファクタ (REVIEW-2026-07 #8) — その1〜その3 完了**。renderer.js 8,050 → 6,158 行。**(その1)** `image-export.js` 新設 (β97 画像書き出し 2 機能: `actionExportAsImage`/`startRegionImageDrag`/`saveRegionImage`/`showImageExportDialog`、~390 行) **(その2)** `save-flow.js` 新設 (確定/上書き/別名の共通パイプライン + ADR-0026 戻す: `actionSave`/`actionExportToPath`/`actionRestoreEditableMaster`/`refreshRestoreMasterUI`/`isPdfOutOfSync`/`refreshDocStateField`、~420 行) **(その3)** `sidebar-thumbs.js` 新設 (最大の塊 ~1,280 行: サムネ描画 observer 遅延 `rebuildThumbs`/`renderThumb`、D&D 並び替え + 外部ファイル/別ウインドウ挿入、＋gap と白紙挿入ダイアログ、複数選択 `sidebarThumbSelection`/`splitThumbSelection`、削除経路 `deleteSelectedPages`、右クリックメニュー `actionSavePagesAsPdf` 含む、サイドバー開閉/タブ切替。`rotatePageBy` は callback 注入で回転系本体は不動、`currentSidebarTab` はタブ snapshot 連携のため get/set export = §4.4 パターン 4、split-save パネルは selection/D&D/gap ヘルパーを import 共有)。**いずれも純移動・機能変更ゼロ** (§4.4 パターン 1 の getter 注入。`workspaceMutated` 書込は setter 注入、menuBar は `setMenuEnabled` callback — early boot の TDZ throw は従来どおり catch)。各 commit で npm test 全緑 (11 スイート fail 0)。タグ無し = CI ビルド未発火・業務実機は v2.0.20 のまま。**残り: その4 = split-view.js (~600 行、sidebar-thumbs のヘルパーを import 共有) → (余力) ページ番号一括/フォーム Tab 順 → 実機一巡**。回転系 (`rotatePageBy` 周辺) は完成領域のため**動かさない**。完了条件 = npm test + 実機一巡 (開く→編集→保存→書き出し→印刷→タブ tearout/dock-back、全抽出後にまとめて実施予定)。**着手動機 = 下記テキスト表示ずれ (§8.2 🆕) の根治 (案 C) を「次の大物」と位置づけたため**。memory [[project_s6_refactor_progress]]
...
renderer.js は中央 orchestrator として残し、機能別モジュールに分散 (B2 完了時 4,472 行 → 2026-07-10 実測 8,019 行に再肥大 → **S6 リファクタ = REVIEW-2026-07 #8 を 2026-07-23 に着手、その1〜その3 抽出後 6,158 行**。残り = split-view 抽出、冒頭の未リリース欄参照):
...
| `sidebar-thumbs.js` | 1367 | サムネ描画 (observer 遅延) + D&D 並び替え/挿入 + ＋gap/白紙挿入 + 複数選択 + 削除 + 右クリックメニュー + サイドバー開閉/タブ切替 — S6 その3 (2026-07-23)。selection/D&D/gap ヘルパーは split-save パネルと共有 |

renderer.js に残る責務: `isOpen` / `projectStore` / `history` / `viewer` 等の core state、`refreshViewer` / `actionOpen` / `openPdfPath` / `actionExport` (別名ダイアログ→save-flow 委譲)、split-view、search、各種メニュー glue。**S6 残り: split-view 抽出のみ** (冒頭の未リリース欄参照)。
...
- **S6 (split-view + sidebar-thumbs 抽出)**: B2 残置責務。**2026-07-23 に着手 (REVIEW #8)** — その1 image-export / その2 save-flow / その3 sidebar-thumbs 抽出済、split-view のみ残り (冒頭の未リリース欄参照)
