...
24. **qpdf Mac/Linux バンドル** — **stable 配布対象に Mac/Linux も含めることが 2026-05-25 に確定** (ユーザー判断)。**2026-06-03 に Linux 完了** (公式 portable v12.3.2 を `vendor/qpdf/linux/` に bin+lib 同梱、`qpdf-sanitize.js findQpdfBinary` を win=flat / mac・linux=bin+lib のレイアウト差対応、`package.json` の mac/linux `extraResources` 設定済)。**残るは Mac バイナリのみ**。**2026-06-05: 配布先 Mac は全て Apple Silicon (M1+)・Intel なしと確認 → arm64 専用ビルドに確定** (arch=`["arm64"]` 反映済)。事務所 M1 で実機ビルド (手順 `QPDF-MAC-TODO.md`、非プログラマと Claude Code 同席、導入 HTML 用意済)。**stable タグ前に必須**。`extraResources` コピー + dmg 実行確認は stable ビルド時に行う
...
2. **qpdf Mac/Linux バンドル** (stable 残務 #5) — Win 同梱済。**2026-06-03 に Linux 完了** (公式 portable v12.3.2 を `vendor/qpdf/linux/` に同梱 + `findQpdfBinary` レイアウト差対応 + `package.json` mac/linux `extraResources` 設定)。**残 = Mac バイナリのみ**。**2026-06-05: 配布先 Mac は全て Apple Silicon (M1 以上)・Intel なしと確認 → arm64 専用ビルドに確定** (`package.json` arch=`["arm64"]` 反映済、universal2/Rosetta/lipo 不要)。実機ビルドは **事務所の M1 Mac** で実施予定 (手順は arm64 前提に更新した `QPDF-MAC-TODO.md`)。**ユーザーは非プログラマのため、M1 に Claude Code を導入して一緒に作業する方針** — 導入ガイド `Mac-ClaudeCode-セットアップ手順.html` を Windows デスクトップに用意済 (memory `[[project-qpdf-mac-arm64-pending]]`)。stable タグ前に必須
3. **クラッシュ診断ロガー一式の撤去** (stable 残務 #6、最後) — β.51-.138 で累積した診断系を撤去。**#2 (qpdf Mac) と現行 β.131-.140 修正の安定確認が完了してから着手** (= 1 週間並走後)。**β.131-.146 は 2026-06-01 の安定確認窓を過ぎ、β.146 まで実機 OK で安定確認はほぼ満たした**ので、残るゲートは実質 **#2 qpdf Mac (Mac ビルド) のみ → その直前 (stable タグを切る直前) に撤去**。撤去対象に β.133 `open-pdf-stage` / `open-pdf-renderer-error` + β.137-138 `print-tick` も含める。**前倒しは NG** (Mac ビルド待ちの間に印刷/Adobe 系が再発しても診断データが取れなくなるため、安全網は最後まで残す)
...
- 🟡 **qpdf Mac/Linux バンドル** (stable 残務 #5) — **Linux 完了 (2026-06-03)**: 公式 portable v12.3.2 を `vendor/qpdf/linux/` に同梱 (SHA256 検証・実機起動確認)、`findQpdfBinary` を win=flat / mac・linux=bin+lib 対応、`package.json` mac/linux `extraResources` 設定。**残 = 🔴 Mac バイナリ**。**2026-06-05: 配布先が全て Apple Silicon (M1+)・Intel なしと確認 → arm64 専用ビルドで確定** (arch=`["arm64"]` 反映済、universal2/Rosetta/lipo 不要)。M1 で `brew install qpdf` → `dylibbundler` 自己完結化 → `otool -L` 検証 → `vendor/qpdf/mac/{bin,lib}/` 配置 → commit。手順は arm64 前提に更新した **`QPDF-MAC-TODO.md`**。非プログラマのユーザーと M1 上で Claude Code 同席作業する方針 (導入 HTML 用意済) + ⚠️ stable ビルド時に extraResources コピー/dmg 実行確認
