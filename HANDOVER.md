...
> - **stable リリース時にやること**:
>   - β51 で追加した **クラッシュ診断ロガーを撤去** (β 期間限定の仮設、ユーザー指示あり)。撤去対象: `crashLogPath()` / `logCrash()` 関数 + 全イベントリスナ + `session-start` ログ + `print-route` / `silent-print-failed` ログ呼び出し + `kpdf3:open-crash-log` IPC + preload `openCrashLog` ブリッジ + index.html の `data-action="open-crash-log"` menu-item + renderer の `actionOpenCrashLog` + menu dispatcher entry + status hint entry。他で使ってなければ `appendFileSync` import も。
> 
