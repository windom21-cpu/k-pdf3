...
最終更新: 2026-05-12（β30 配布フェーズ） / 現在のバージョン: **v2.0.0-beta.30（β28 後の試し置き UX 連投修正 = C8〜C12 着地、業務実機テストフェーズ継続中）**
...
> 
> **β30 時点での未解決テスター要望**:
> - **C3 Adobe で押したスタンプ (annotation) が viewer で見えない（印刷では出る）**: HANDOVER §15.3 の annotation read-only proxy に該当、新セッション規模なので別セッションで取り組む方針。実害は viewer 表示のみで印刷物には出るので業務影響は中。
> 
...
- **開発リポ**：[windom21-cpu/k-pdf3](https://github.com/windom21-cpu/k-pdf3)
  - デフォルトブランチ：`main`
