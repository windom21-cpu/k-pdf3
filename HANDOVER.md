...
- **v2.0.27-beta.6** = **分割サムネで Ctrl+A (Cmd+A) → 全サムネ選択 + Electron テストランナーの静かな早期終了を修正** (feat `bac2036`・test `2812e43`・release `533ce2b`、2026-08-29、Windows β 配信 — **実運用で問題なし → 2026-08-29 v2.0.27 stable に昇格**)。**(A) Ctrl+A** (ユーザー要望 2026-08-29): 分割パネル表示中、テキスト入力欄以外での Ctrl/Cmd+A で `#split-flow` の全サムネ (白紙等の synthetic 含む・見た目順 = Shift+click が跨げる集合と同じ) を選択し、ステータスバーに「分割サムネ N ページを全選択しました」。`explicit=true` にするので右クリック 保存/印刷/削除 と印刷ボタンの範囲 preselect にそのまま乗る。**実装**: `sidebar-thumbs.js` (`splitThumbSelection` の持ち主) に `selectAllSplitThumbs()` + document レベル keydown。分割モード中はビューア/サイドバーが `display:none` なので、renderer.js の overlay 全選択 (window リスナー、β.107) より先に bubble で拾って `stopPropagation` — 両者は競合しない。通常表示の左サイドバー Ctrl+A は**未実装のまま** (要望があれば同型で追加)。**(B) テストランナー事故 (副産物・重要)**: v2.0.24 で `renderer-load-smoke.mjs` を `ELECTRON_TESTS` の先頭に置いて以降、その BrowserWindow の `destroy()` → Electron 既定の `window-all-closed` → quit (exit 0) で **`npm run test:m1` が smoke の直後に静かに終了し、m1 / m3 / workspace-cleanup / source-encrypted-flag / workspace-portability / stamp-export-import (+ β5 の inserted-page-order) が約 1 か月一度も走っていなかった** (npm test は緑のまま)。`test/electron-runner.cjs` に `app.on("window-all-closed", noop)` を足して runner 自身の `app.exit()` まで生かす。8 本 199 項目は現行コードで全通過 = 隠れ回帰なし。教訓: **npm test が緑でも Electron ランナー分は「=== Result」行が 8 本ぶん出ているかを見る**。**実機確認チェックリスト**: ①分割ビューで Ctrl+A → 全サムネが選択色 (白紙・挿入ページ含む) ②その状態で右クリック 保存/印刷/削除 が全ページ対象 ③白紙ダイアログ等の入力欄にカーソルがあるときの Ctrl+A は文字全選択のまま ④分割を閉じて通常表示の Ctrl+A は従来どおり overlay 全選択。memory [[project_split_ctrl_a_and_runner_fix]]
- **v2.0.27-beta.5** = **未保存の追加ページ (外部 PDF/画像/Word/別ウインドウ D&D) の直後の＋で入れた白紙が、追加ページの前に回り込む** (fix `df1778c`・release `198a324`、2026-08-29、Windows β 配信 — **実運用で問題なし → 2026-08-29 v2.0.27 stable に昇格**)。ユーザー報告「ページを追加した後に、保存せずそのまま追加したページの後に白紙ページを差し込むと、追加ページの前に差し込まれてしまう」。HANDOVER に同件の記録なし → **合成データで再現を確立してから修正** (Workspace を直接叩く再現スクリプト: p1 の後に外部 2 ページ → `-1@1.333 / -2@1.667`、-2 の後ろの＋で白紙 → `-3@1.502` が -2 の**前**に落ちる)。**根因**: 追加ページは β77 の仕組みで**区間を等分した小数 display_order** を持つが、白紙 (`addInsertedPage`) は display_order を持たず `Workspace.getPages` のスロット由来キー `anchor + 0.5 + orderInSlot×0.001` で並ぶため、その間に挟まる。確定保存すると追加ページが source 化されるので「保存前だけ」に見える (複数追加なら 2 枚目の前・1 枚追加なら偶然正しい)。**修正 (afterKey 未指定の旧経路は不変)**: renderer の＋gap が既に持つ `afterKey` (見えている直前ページの key、正=source / 負=synth / 0=先頭) を `kpdf3:add-inserted-page` に同乗 → main が `Workspace.visualGapAfter(afterKey)` (外部 PDF 挿入 `_insertPdfBytesIntoWorkspace` の lower/upper 算出をそのまま抽出した共用ヘルパ、ロジック不変) で隣接 2 ページの orderKey を取り**中点**を `displayOrder` として `inserted_pages.display_order` に保存。store/workspace の `addInsertedPage` に `displayOrder` を追加 (省略時は従来どおり NULL)。サイドバー・分割ビューの両 gap 経路に適用。**テスト**: `test/inserted-page-order.test.mjs` (Electron ランナー登録、12 項目: 旧経路での再現ガード / 修正後の並び / 追加 1-2 枚目の間 / 先頭・末尾・通常ページ間 / 並び替えで slot から離れた追加ページの後ろ / 旧経路の display_order NULL 不変)。⚠️ このテストは beta.6 のランナー修正まで npm test で走っていなかった (単体は `ELECTRON_RUN_AS_NODE=1 npx electron test/inserted-page-order.test.mjs` で通過確認済み)。**実機確認チェックリスト**: ①外部 PDF (2 ページ以上) をサムネ間の＋にドロップ → 保存せず、追加した最後のページの後ろの＋で白紙 → 追加ページの直後に入る ②追加 1 枚目と 2 枚目の間の＋ → その間 ③画像 / Word 追加でも同様 ④分割ビューのサムネ列の＋でも同様 ⑤従来どおり: 先頭の＋ / 通常ページ間 / 最終ページの後ろ / 白紙の後ろにもう 1 枚 ⑥その状態で別名保存 / 下書き保存 / 印刷のページ順が画面と一致。memory [[project_blank_after_added_page_order_fix]]
- **v2.0.27-beta.4** = **A4 サイズに切り取りを「選択した *その* 1 ページだけ」原則に変更** (feat `48a60ac`・docs(adr) `8cdeebe`・release `163200f`、2026-08-25、Windows β 配信 — **実運用で問題なし → 2026-08-29 v2.0.27 stable に昇格**)。ユーザー報告「1 つのファイル中に複数の切り出し可能なページがあるとき、いまはそのすべてのページについて同じ枠の位置で一斉に切り出して、新しくできたファイルは複数の切り出したページを 1 つのファイルにして出力される。選択した『その』ページだけの切り取りを原則にして、対象となるページ全てに適用するのは例外的に明示のときだけにしたい」。**β3 の実装は「ダイアログを開いた時点で全非 A4 ページに初期枠を配る」(そのまま保存 = 全ページ一斉切り取り) だったのを反転**: **①初期枠は右クリックした開始ページ (`primaryPageNo`) にだけ置く** — 他の非 A4 ページは「前/次の対象」で巡回はできるが枠なし = 切り取らない (巡回先で「枠を追加」すれば個別に対象化。ページラベルに「枠 2」/「枠なし: 切り取らない」を表示) **②例外の一括はダイアログ内チェックボックス「同じ枠位置を他の非 A4 ページにも適用する（他に N ページ）」を明示 ON にしたときだけ** (寸法が同じページは枠座標をそのままコピー、A3/B4 混在など違うサイズはそのサイズの既定枠。ON 後に個別ページで手調整したページ = `touched` は 以降の再配布で上書きしない。OFF に戻すと他ページの枠は落ちて「その 1 ページだけ」に戻る。対象が 1 ページの文書ではチェックは無効化) **③出力は「枠を置いたページだけ」の新ファイル** — `composePagesForExport` に枠付きページだけを渡す形 (= `actionSavePagesAsPdf` のページ部分保存と同型。A3 横 1 ページを切れば出力は A4 縦 2 ページだけ)。**副作用として、出力に含まれないページとそのしおりは付いてこない** (部分保存と同じ性質。文書全体を残す構成が要るなら保存前に選ばせる形を追加する余地あり)。1 ページだけ切ったときの初期ファイル名は `元名_pN_A4.pdf` (挿入ページは `元名_insertedN_A4.pdf`、複数ページなら従来どおり `元名_A4.pdf`)。**main 側 (`applyCropFramesToPdf` / `expandPageOrderForCrop`)・`_assembleHybridPdfOnce`・回転ベイク (`rotate-place.js`)・byte-copy は 1 バイト不変** (cropFrames は payload の pages 順で index を引く契約なので、部分集合を渡しても main 側の契約は変わらない。0 枠ページ素通しの経路も残っている)。変更は `src/renderer/crop-a4.js` (原則の反転・propagateFromPrimary / noteFrameEdit / updatePageLabel の追加・保存時の rows 絞り込み)、`index.html` (チェックボックス行 + 説明文)、`style.css` (`.crop-a4-options`)、ADR-0029 §3/§4 の追記のみ。npm test 全 16 スイート fail 0。**実機確認チェックリスト**: ①A3 横ページを右クリック →そのまま「切り取って保存」→ **出力が A4 縦 2 ページだけ** (他ページが混ざらない — 本丸) ②非 A4 ページが複数ある文書で開いたとき、他ページに枠が入っていない (「次の対象 ▶」で回して「枠なし: 切り取らない」表示) ③チェック ON → 他ページに同じ位置の枠が入る / その状態で保存すると全対象ページぶんが 1 ファイルに出る ④チェック ON の後に 1 ページだけ枠をずらす → 他ページは巻き添えにならない・その後に開始ページの枠を動かしても手調整したページは動かない ⑤チェック OFF に戻す → 他ページの枠が消えて開始ページだけになる ⑥巡回して手で「枠を追加」した 2 ページを保存 → その 2 ページぶんだけが出る ⑦回帰 (β3 のチェックリストと同じ): 回転ページ / 吹き出し・マーカー付きページの切り出し位置、A5 の白余白 A4 化、セキュア書き出し・白黒との併用、新タブで開く。memory [[project_crop_a4_feature]]
- **v2.0.27-beta.3** = **A4 サイズに切り取り (ADR-0029) + 「名前を付けて保存」名前欄のクリック選択維持** (fix `054c4dd`・feat `cbbecba`・release `8dee302`、2026-08-24、Windows β 配信 — **実運用で問題なし → 2026-08-29 v2.0.27 stable に昇格**)。**(A) A4 サイズに切り取り (新機能)**: 動機 = ユーザー「A3 横の PDF を A4 縦 2 枚に分割したい」。サムネ右クリック「A4 サイズに切り取り…」→ 文書内の全非 A4 ページ (canonical 寸法が A4 縦 ±2pt でない。A4 横も対象) を 1 ページずつ巡回するダイアログで、**A4 縦固定サイズの枠** (ドラッグ移動のみ・リサイズなし、ページ端/半分位置スナップ) を配置 →「切り取って保存」で**枠 1 つ = 出力 1 ページ**の別ファイルを作り新タブで開く。**元ファイル / workspace は不変** (書き出し系機能として実装 — 可逆確定 / byte-copy / overlay 座標には一切乗らない)。初期枠は **A3 横だけ左右 2 枠** (ちょうど A4 縦 2 枚ぶん)、他サイズは中央 1 枠 +「枠を追加/削除」(B4 横なども 1 枠スタート = ユーザー決定)。A4 未満のページは枠がグレー背景にはみ出して**白余白付き A4 化**。枠 0 のページは素通し。保存はいつもの保存ダイアログ (セキュア書き出し / 白黒トグル付き、初期名 `元名_A4.pdf`)。**実装**: 既存 IPC `kpdf3:export-pdf-rasterized` の payload に `cropFrames` ({pageNo:[{x,y,w,h}]}、canonical top-left pt) を追加し、**組み立て後の後処理** `src/main/crop-a4.js` (`applyCropFramesToPdf`) が embedPage + drawPage で A4 ページに差し替える — **`_assembleHybridPdfOnce` / 回転ベイク (`rotate-place.js`) / byte-copy は 1 バイト不変** (cropFrames 無しの呼び出しは従来と完全同一)。⚠️ 組み立ての verbatim 高速パスは intrinsic /Rotate を持ったままページを運ぶ (sourceRot≠0/userRot=0 等) ため、後処理側が `rotatedSourcePlacement` で /Rotate をベイクしながら配置して吸収 (mupdf ピクセル検証済み)。しおりは `expandPageOrderForCrop` で切り取り後の構成に付け替え (2 枠目以降は null 埋め = 先頭枠に乗せる。pageNo を重複させると Map の後勝ちで最後の枠に乗る罠)。白紙ページ (Contents なし) は pdf-lib embedPage 不可 + **throw が save() まで遅延**するため `page.node.Contents()` の事前チェックで空 A4 に fallback。プレビューは exporter 側 `composeSinglePageCanvas` = 出力と同一描画 (viewer/exporter 二重実装問題を新 UI に持ち込まない)。UI は `src/renderer/crop-a4.js` (循環 import 回避のため actionCropToA4 は renderer.js から sidebar-thumbs へ注入 — v2.0.23 TDZ 事故の教訓)。**(B) 名前欄クリック**: 保存ダイアログを開いた直後は名前欄の stem が事前選択されているが、focus 済み input へのクリックの mousedown 既定動作 (キャレット配置) で選択が即解除されていた → ワンショットフラグで**最初の 1 クリックだけ既定動作を吸収し、2 クリック目から通常動作** (`file-browser.js`。タイプ後 / blur 後は吸収しない。別名保存 / 画像書き出し / ページ保存 / 分割保存の全経路共通)。テスト: `test/crop-a4.test.mjs` 5 件 (A3 左右分割 / /Rotate=90 ベイク / A5 余白 A4 化 / 素通し + pageOrder 展開 / 入力正規化 — mupdf ピクセル検証)、npm test 全 16 スイート fail 0 (renderer-load smoke 含む)。**実機確認チェックリスト**: ①A3 横 → そのまま「切り取って保存」で左右ぴったり 2 枚 (本丸) ②枠ドラッグ / スナップ / 追加・削除の操作感 ③スキャン由来の横向きページと、アプリで回転させたページの両方で切り出し位置が枠と一致 (経路が別) ④吹き出し・マーカー付きページが枠どおりに切れる (WYSIWYG) ⑤A5 → 白余白付き A4 ⑥しおり付き文書で飛び先維持 ⑦セキュア書き出し / 白黒との併用 ⑧名前欄: 開いた直後クリック → 選択維持、再クリック → カーソル配置、先にタイプ → 従来どおり。memory [[project_crop_a4_feature]]
- **v2.0.27-beta.2** = **現在ページ判定の両端規則 — 文書末尾が見えていれば最終ページ / 先頭が見えていれば 1 ページ目** (fix `0798e3a`・release `c30bf06`、2026-08-22、Windows β 配信 — **末尾項目は実機確認済み 2026-08-22 OK「ちゃんと最終ページにスクロールしたときにサムネの選択が最終ページになった」、他項目も実運用で問題なし → 2026-08-29 v2.0.27 stable に昇格**)。beta.1 実機報告「最終ページを末尾まで表示させてもサムネの選択 (紺色の現在枠) が最終ページにならない」。**根因**: 最下部 clamp では最終ページ下端が viewport 下端に張り付く (`.viewer-container` は `padding: 16px 0`) ため、最終ページの高さが viewport × 2/3 未満 (横長ページ・縮小表示・縦長画面) だと beta.1 の 1/3 probe が前ページに落ちる。beta.1 の fallback は「probe が全ページより下」しか拾っていなかった。対称に、先頭ページが viewport × 1/3 より低い縮小表示では scrollTop=0 でも 2 ページ目が current になる穴もあった。**修正** = `currentPagePos` (`src/domain/page-registry.js`) に両端規則を追加: ①scrollY ≤ 0 → 1 ページ目 ②scrollY + viewportH ≥ totalHeight かつ最終ページが viewport に掛かる → 最終ページ (これ以上 scroll できない以上、末尾を見ているとみなす) ③それ以外は probe 判定。**viewer.js は不変**。`test/page-registry.test.mjs` [13] に短い最終ページ (842/842/200、viewport 900 の clamp) の再現 (旧規則 `visiblePageRange` = 2 ページ目 / 新規則 = 3 ページ目) を追加、npm test 全スイート fail 0。注意: サムネの**紺色背景** (`.is-current`) が scroll 追従する「現在ページ」、**破線枠** (`.is-selected`) は Ctrl/Shift クリックの「選択」で scroll では動かない (従来どおり)。
- **v2.0.27-beta.1** = **「現在ページ」判定を viewport 上端基準 → 上端 + 高さ 1/3 の点に変更 + scrollToPage の pin を全件に拡大** (fix `791727e`・release `0e1d5b3`、2026-08-22、Windows β 配信、**`releaseType` を `prerelease` に戻した** — **実機: 末尾で最終ページにならない報告 → beta.2 で修正。他項目も実運用で問題なし → 2026-08-29 v2.0.27 stable に昇格**)。ユーザー報告「しおりを付ける時や 1 ページだけ回転させようとしたとき、選択されていたページが一つ前のページであることがよくある」→「紙のどのあたりまで scroll したとき current になるのか」の確認依頼から。**旧規則** (v1 から不変): current = **「viewport 最上端に掛かっているページ」** (`visiblePageRange().first`、scroll listener は rAF キャッシュ `_currentPage`、回転は同期読み `visiblePageNow()`、どちらも同規則) — 前ページの下端が画面上端に 1px でも残れば前ページのまま = 次ページの本文を読み始めていても current は前ページ。ページ間 8px の隙間は次ページ側。25-beta.3 / 23-beta.3 の clamp 修正は特殊ケース対処で規則自体は触っていなかった。**規則変更の影響範囲 (事前調査、`viewer.currentPage` / `visiblePageNow` / `onPageChange` の全消費者)**: しおり追加 (`bookmark-pane.js`) / 回転ターゲット `resolveRotationTargets` / 分割サムネ自動選択 (25-β2) / 印刷ダイアログ「現在のページ」+ 初期値 / 画像書き出し「現在のページ」/ 別窓ポップアップ / Ctrl+V フォールバック (`activePage`) / ステータス N/M・Ctrl+G 初期値 / 左サムネ現在枠 (`highlightCurrentThumb`) / メニュー前・次有効判定 / fit-width・fit-page の基準ページ (β76 `recenterCurrentPageHorizontally`)。**壊れやすい所**: 前/次・Ctrl+G・ページ番号入力は `scrollToPage(N)` = N の上端を viewport 上端に揃える作りなので、判定点を下げるとページ高さ < viewport × 1/3 の縮小表示で current が N+1 になり「次へ」で 1 ページ飛ぶ / 入力と表示が食い違う。**採用案** (「viewport 中央のページ」「最大面積のページ」は scrollToPage との整合が崩れるため不採用): ①`src/domain/page-registry.js` に純関数 **`currentPagePos(layout, scrollY, viewportH, probeRatio)`** を新設 (probeRatio=0 が旧規則と等価。`visiblePageRange` 自体と既存テストは不変)。viewer.js の scroll listener と `visiblePageNow` が **`CURRENT_PAGE_PROBE = 1/3`** で同じ関数を使う (β10 の「同期 scrollTop 読み」は不変 = 速い scroll 直後の回転レース耐性維持。**2 箇所は必ず同じ規則にすること**) ②25-beta.3 の `_clampPin` (最下部 clamp 専用) を **`_navPin`** に改名し、`scrollToPage` 全件で「行き先ページが見えていれば current として採用・pin し、scrollTop がそこから動くまで維持」に拡大 — サムネクリック / 前・次 / Ctrl+G / しおり / 検索ジャンプ / 回転後の復帰 (`scrollToPage` 呼び出し 16 箇所すべて) が縮小表示でもずれない。手 scroll で即・通常判定へ戻るのは従来どおり、クリック anchor 破棄の副作用も維持。**体感**: 前ページの尻尾が画面上 1/3 より上に消えた時点で次ページが current (前ページの最後の数行を読んでいる間はまだ前ページ)。**描画 (IntersectionObserver) / exporter / 保存 / 印刷パイプライン / `rotatePageBy` / byte-copy は 1 バイト不変**。旧挙動に戻すなら `CURRENT_PAGE_PROBE = 0` (両端規則を除き等価)。テスト: `test/page-registry.test.mjs` [11]〜[14] (旧規則等価性 / 1/3 規則 / 末尾規則 / 隙間・400 ページ) 追加、npm test 全スイート fail 0 (renderer-load smoke 含む)。**実機確認チェックリスト**: ①次ページ本文を読み始めた状態でしおり追加 / ↺↻ → そのページに乗る、N/M も一致 (本丸) ②尻尾が画面上 1/3 以上残っていればまだ前ページ ③前/次連打・Ctrl+G・ページ番号入力が 100% と縮小表示 (50% 以下、ページが画面の 1/3 未満) の両方で行き先と N/M 一致・1 ページ飛ばない ④最終ページ: サムネクリック → N/N、分割 → 最終ページ選択、回転 → 最終ページ (25-β3 回帰)、手で最下部まで scroll → 最終ページ (**beta.1 では NG → beta.2 で修正・実機 OK**) ⑤回帰: サムネクリック後に大きく scroll → 回転が見ているページに乗る / 左サムネ枠の追従 / 分割自動選択 / 印刷「現在のページ」/ Ctrl+V 先 / A3・A4 混在 PDF の fit-width 中央寄せ。
- **v2.0.26 (stable、3 OS)** = **v2.0.25-beta.1〜3 + v2.0.26-beta.1〜4 の昇格** (2026-08-20、release コミットで `releaseType` を `release` に復帰)。全 7 本をユーザーが実機で確認してから昇格 (v2.0.23 の教訓を遵守)。内訳は上記「現在のバージョン」参照。
- **v2.0.26-beta.4** = **タブが増えて見切れたとき、見切れたタブを選べるように** (fix `27882ac`・release `ff8cb5f`、2026-08-20、Windows β 配信 — **実機確認済み 2026-08-20 OK → v2.0.26 stable に収録・クローズ**)。ユーザー報告「タブが増えて見切れるようになった時、見切れたタブが選択できない」。**原因**: `#tab-list` は `overflow-x: auto` だがスクロールバーをユーザー要望で非表示にしており (`scrollbar-width: none` / `::-webkit-scrollbar { display: none }`)、縦ホイールは横スクロールに変換されないため、溢れた右端のタブに届く手段が Shift+ホイール以外になかった (新規タブは右端=画面外に生まれる)。**対処 (`src/renderer/tab-manager.js` 末尾の「Tab-bar overflow」節 + `index.html` / `style.css`、既存のタブ DOM・D&D 並び替え・別ウインドウ経路は不変で追加のみ)**: ①タブ列上の縦ホイール → 横スクロール (deltaX 優勢ならネイティブに任せる) ②`renderTabBar` のたびに `scrollActiveTabIntoView()` (bounding rect 差分で list の scrollLeft だけ動かす、祖先はスクロールしない) ③溢れているときだけ `◀ ▶` スクロールボタン (端で disabled) と `▼`「すべてのタブ」一覧メニュー (`#ctx-tab-list`、✓=現在 / ●=未保存、ボタン直下に右寄せ) を表示 — `updateTabOverflowUi()` は scroll / ResizeObserver (rAF 遅延) / render で更新、表示判定は「ボタン表示状態で収まるか」の厳しい側なので振動しない ④タブ D&D 中に list の端 28px 内で自動スクロール。**落とし穴**: `#tab-list` は `#tab-bar` の直接の子を維持すること (renderer.js `reportTabBarRect` が `list.parentElement` を .tab-bar として読む、B3-γ の別ウインドウ dock 判定に使う) / `[hidden]` な button は 98.css の display に負けるので `display: none !important` を明示 / `◀ ▶` は smooth ではなく即時スクロール (非表示ウインドウでは smooth が進まず、Win9x 風には不要)。**検証**: `npm test` 全通過 + WSL の Electron で index.html を実ロードし 30 タブ溢れ → ボタン表示 / ホイール / ▶ / 端の disabled / ▼ 開閉 / タブ削減でボタン消失を自動確認。
- **v2.0.26-beta.3** = **確定/別名保存で workspace しおりが 0 件のとき、元 PDF の /Outlines を書き戻す** (fix `8b9abf0`・release `1f3179d`、2026-08-18、Windows β 配信 — **実機確認済み 2026-08-20 OK → v2.0.26 stable に収録・クローズ**)。beta.2 の実機 OK 後に残していた穴を、ユーザー決定「**しおりを意図的に全部消す運用は無い**」を前提に塞いだもの。**構造**: `export-pdf-rasterized` は workspace しおりが 0 件だと /Outlines の write-back を丸ごとスキップする設計だったが、**pdf-lib 再合成は元 PDF の /Outlines を運ばない** — つまり 0 件のまま確定するとファイルからしおりが消える。beta.2 の事故 (下記) で助かったのは、確定元のマスター workspace にしおりが 38 件残っていたからにすぎない。0 件の実態は「取込前」か「取込失敗」しかないので、**元 PDF のしおりをそのまま書き戻す**。**副作用**: 全消し → 保存で復活する (上記ユーザー決定により許容、コード内にも明記)。**実装**: ①`outlineToFlatBookmarks()` (`src/backend/pdf-outlines.js`) = OutlineNode 木 → `addFlatOutlinesToPdf` が食う flat 形 (ページ無し見出しは親のページを継承 = renderer の手動［取込］と同じ規則) ②`readSourceOutline(ws, {preferQpdf, bytes})` (main.js) にしおり読取を集約し get-outline IPC と保存経路で共有。**保存経路は `preferQpdf: true`** — 出力 PDF を抱えたまま 700MB の確保を試すのは危険なので、サイドカー実体があるときは安い qpdf から入る。既に読んである `sourceBytes` を渡して再読込も回避 ③元しおりが削除ページを指す場合は既存の prune + 子繰り上げ規則がそのまま効く。`test/outline-rescue.test.mjs` 5 件追加 (往復一致 (階層 + CJK) / **救済しないとしおりが消えること自体の見張り** / 削除ページの繰り上げ / 境界)。npm test 全スイート fail 0 (skip 1 は既存)。**実機確認チェックリスト**: ①しおり入り PDF を開いて**すぐ** (しおりタブを見ずに) ページ追加 → 上書き確定 → Adobe でしおり全件生存 ②741MB の実機ファイルでも同じ ③しおりをリネーム/追加してから確定 → **その編集内容が**出力に載る (救済が編集を上書きしていない) ④しおりを何件か削除して確定 → 削除分は消えたまま (0 件でない限り救済は発動しない) ⑤回帰: 元からしおり無しの PDF の確定 (余計なしおりが生えない) / 別名保存 / byte-copy (無編集の上書き)
- **v2.0.26-beta.2** = **巨大 PDF でしおりが「全部消えた」ように見える問題 — /Outlines 読取の qpdf fallback + 失敗の可視化** (fix `8911373`・release `0c152a7`、2026-08-18、Windows β 配信 — **実機確認済み 2026-08-18 OK → クローズ**)。**実機事故**: 741MB の記録 PDF (裁判所謄写) にページ追加 → 上書き (確定) 保存 → しおりが全件消えて見え、［編集に戻す］も `malloc (740227416 bytes) failed`。**しおりは実際には消えていなかった** — 保存ファイルの /Outlines 38 件も、確定元のマスター workspace (`20260717-210228_bdd46b7d`) の bookmarks 38 件も無傷で、消えたのは確定後の新フラット workspace の表示だけ。**根因は 1 つ**: 元 PDF を丸ごと Buffer に読み、さらに mupdf の WASM ヒープへコピーする経路が 700MB 級で OOM する。確定後の自動取込 (`autoImportOutlinesIfEmpty` → `getOutline` → `extractOutline`) がこの OOM で**無言失敗** (console.warn のみ) → workspace しおり 0 件 → ペインは書き戻された /Outlines のフォールバック表示に落ち、しおりを 1 件足した瞬間に「全部消えた」状態になる (v2.0.23-beta.1 で直した症状の再来だが穴は別: 前回=自動取込を呼んでいない / 今回=呼んでいるが落ちている)。`restore-editable-master` も `openPdfDocument(bytes)` で同じ壁。**対策 (2 段構え)**: ①**逃がす** — `kpdf3:get-outline` を try/catch し、mupdf が落ちたら同梱 qpdf にファイル経路で読ませる (`src/main/qpdf-outline.js`、`--json=2 --json-key=outlines`)。qpdf は逐次読みなのでメモリを食わず、**実機の 741MB でも 0.1 秒で 38 件**を復元。`Workspace#getSourcePath()` を追加 (β.134 サイドカー実体の解決、getSourceBytes と同じ stale-path 救済つき)。既存 mupdf 経路は不変で throw 時だけ通る追加分岐 ②**黙らせない** — `autoImportOutlinesIfEmpty` が `{ok, imported, error}` を返すようにし、確定保存後 (save-flow) と PDF オープン時 (renderer) の両方で失敗をステータス表示。`refreshBookmarks` も throw を握って「(しおりを読み取れませんでした)」を出す (**無言 0 件はしおり消失と見分けが付かない**、が今回の教訓)。［編集に戻す］の OOM も生エラーではなく「他のタブを閉じて再起動し、開いた直後に再試行」の案内文に。`test/qpdf-outline.test.mjs` 5 件追加。**消えたしおりの復旧レシピ** (同種事故が起きたとき): アプリ終了 → qpdf `--json=2 --json-key=outlines <保存ファイル>` でしおりを吸い出し → 該当 workspace の `bookmarks(id,parent_id,title,page_no,sort_order)` へ直接 INSERT (実施前に .kpdf3 を退避)。今回はこれで 38 件を復元し、実機で確認済み。**残していた穴 (0 件のまま再確定するとファイルからしおりが消える) は beta.3 で対処**
- **v2.0.26-beta.1** = **OCR 済み PDF のテキスト選択・コピー (文字選択モード) + マーカー「範囲」タイプの I-beam カーソル化** (feat `4b59e9f`・release `a3e89ac`、2026-08-08、Windows β 配信 — **実機確認済み 2026-08-20 OK → v2.0.26 stable に収録・クローズ**)。ユーザー要望「OCR された PDF を選択してコピーできるようにしたい」。ページは canvas (画像) 描画のため従来は文字を選択できなかった。**ユーザー決定 2 点**: ①対象は「すでに OCR 済みの PDF」のみ (アプリ内 OCR 機能は作らない — スキャン PDF の文字認識は別プロジェクト級) ②操作はツールバー「文字選択」ボタンで切替 (常時有効は既存クリック操作と衝突するため不採用)。**方式 = pdf.js textLayer と同じ透明テキストレイヤー** (追加パッケージなし — mupdf 1.27.0 同梱の structured text を使用): `backend/mupdf-text.js` (新規) の `extractPageTextLines` が行単位 box を **fitz 空間 (post-/Rotate・page bounds 左上原点 = pixmap と同一空間)** で返し、main `kpdf3:get-page-text` (render-page と同じ activeForEvent 流儀、synthetic page は空)、`renderer/text-select.js` (新規) が `placementMode "select-text"` でページ canvas の上に透明文字レイヤーを構築。**設計要点**: 可視ページのみ IntersectionObserver で構築・pageNo キーで cache (point 座標なので zoom 非依存、タブ切替/PDF 差し替えは refreshViewer の reset で破棄) / userRotation は canvas 描画と同じ「中央合わせ + rotate」でレイヤーごと回すので回転ページでも位置一致 / 行 = block 要素なので複数行コピーに改行が入る / 実測幅→bbox 幅の scaleX で当たり位置補正 / 縦書き行 (wmode=1) は writing-mode: vertical-rl の簡易対応。**UI**: コピーは Ctrl+C (選択中は overlay コピーより優先で native に譲る分岐) と右クリック「選択テキストをコピー」(選択中のみ表示)、右クリックメニューに「テキスト選択」も追加、Esc で解除。モード中は `.overlay` を pointer-events: none にして文字ドラッグ優先 + オプションバー非表示。**墨消し済みページは文字が物理削除済み (β.85 真の墨消し) なのでコピーで漏れない**。未 OCR ページは選択対象が無いだけでエラーにしない。**既存経路への変更は viewer.js `_buildPageDoms` 末尾の `kpdf3:pages-rebuilt` CustomEvent 通知 1 行のみ — exporter / 保存 / 印刷 / 回転 / byte-copy は 1 バイト不変** (表示専用の追加レイヤー)。同乗: マーカー「範囲」(行で塗るタイプ) のカーソルを crosshair → I-beam に (`syncMarkerCursor`、行を狙う操作のため。「直線」は crosshair のまま、マーカーモード中の種類切替にも追従)。テスト: `test/mupdf-text.test.mjs` 3 本 (フラット bbox / Rotate90 空間 / テキストなし) を npm test チェーンに追加、全スイート fail 0 (renderer-load smoke 含む)。**分割ビュー・別窓ポップアップは v1 対象外** (本体ビューアのみ。要望が出たら拡張)。**実機確認チェックリスト**: ①OCR 済み PDF で文字選択 → Ctrl+C → メモ帳貼り付け (文字化け・脱落) ②選択ハイライトの位置ずれ (ズーム 50/100/200%) ③回転ページでの選択位置 ④モード中に誤配置しない / 解除後は従来どおり配置できる ⑤タブ切替直後に前 PDF の文字を拾わない ⑥回帰: 印刷・別名保存・確定保存・サムネ (経路不変だが各 1 回) ⑦マーカー「範囲」= I-beam /「直線」= crosshair、種類切替で即追従
- **v2.0.25-beta.3** = **最終ページが「現在ページ」になれない問題の解消** (fix `a2b9889`・release `01a7f4f`、2026-08-02、Windows β 配信 — **実機確認済み 2026-08-20 OK → v2.0.26 stable に収録・クローズ**)。beta.2 実機 OK 後のユーザー報告「最終ページを選択して分割画面に行けない・最終ページだけの回転ができない (前からの不具合かも)」。**根因は v2.0.23-beta.3 と同じ最下部 clamp**: viewport より背の低い最終ページはスクロールがページ先頭に届かず、現在ページ判定 (= viewport 最上部) が 1 つ前のまま — つまり**最終ページは構造的に current になれなかった**。v2.0.23-beta.3 は回転ターゲット側だけ症状回避したが、beta.2 の分割自動選択が `viewer.currentPage` を使うため前ページを選択し、さらに `resolveRotationTargets` は分割選択を最優先するので**分割画面での回転も前ページに乗る**形で顕在化 (縮小表示では最終ページ以外でも起きる)。**修正**: ①`viewer.scrollToPage` に **`_clampPin`** — clamp で届かなくても行き先ページが見えていれば current に採用して onPageChange 発火し、scrollTop がその位置から動くまで scroll listener の「最上部ページ」巻き戻しを抑止 (クリック anchor 破棄の既存副作用は維持)。`visiblePageNow` も pin を尊重 (**同期 scrollTop 比較なので β10 の「scroll 直後 rAF 未反映」レース耐性は不変**。手でスクロールした瞬間に通常判定へ戻る)。unload でクリア。サムネクリックだけでなく **PageDown / Ctrl+G / しおり / 検索ジャンプで最終ページへ行く経路も同時に直る** ②分割自動選択 (beta.2) に v2.0.23-beta.3 と同規則の保険: 単一のサイドバー選択が viewport 内に見えていれば current より優先 (複数選択・画面外 stale 選択は従来どおり無視)。**`rotatePageBy` / `resolveRotationTargets` 本体は 1 バイト不変** (回転系不動ルール)。npm test 全スイート fail 0。**実機確認チェックリスト**: ①最終ページを左サムネクリック → ステータス N/N + 現在枠が最終ページ ②その状態で分割ボタン → 分割側で最終ページが選択・中央表示 ③分割のまま回転 → 最終ページだけ回る ④分割に行かず回転 → 最終ページだけ回る (v2.0.23-beta.3 回帰) ⑤PageDown 連打 / Ctrl+G 最終ページ指定 → N/N ⑥回帰: 最終ページから手でスクロールで戻る → 現在ページ表示が追従して戻る (pin 解除) / 中間ページのサムネクリック / 複数選択一括回転 / 縮小表示でサムネクリック → クリックしたページが current
- **v2.0.25-beta.2** = **分割サムネを開いたとき、いま見ていたページを選択して中央に表示** (feat `672c409`・release `9dad091`、2026-08-02、Windows β 配信 — **実機確認済み 2026-08-02 OK「問題なく実行できている」→ クローズ**)。ユーザー要望「スクロールで見ていたページを、分割サムネ移行時に選択してその位置を表示してほしい」。`actionSplitSave` の rebuildSplitUI 後に**追加分岐**: `viewer.currentPage` のサムネを `splitThumbSelection` に単独選択 (plain クリックと同じ非明示選択 = 前セッションの explicit 残りもリセット) + `scrollIntoView (block: center)`。observer 生成前にスクロールするので、見ている位置のサムネから優先生成される副次効果あり。`refreshSplitView` (split 中の再構築) は従来どおり選択復元のみで不変。**仕様上の注意**: 選択の意味は plain クリックと同一なので、分割を開いた直後に印刷するとそのページ 1 枚が「分割画面選択」として preselect される (範囲欄に表示されるので気付ける。違和感が出たら「自動選択は印刷 preselect に乗せない」調整が可能)。→ **最終ページで前ページが選択される問題が発覚 → beta.3 で修正**。npm test 全スイート fail 0。
- **v2.0.25-beta.1** = **ページ跨ぎスクロールのカクつき軽減** (fix `4ff2c29`・release `073f9b7`、2026-08-02、Windows β 配信 — **実機第一印象「だいぶいい」(同日ユーザー明言) → 様子見中、問題なければ stable 昇格**)。ユーザー報告「ページを跨ぐ際に一瞬カクつく・止まる」— HANDOVER に前例のない未着手領域。**根因は「境界を跨ぐ瞬間」に重い処理が 2 系統重なる構造**: (A) ページの遅延 render が rootMargin 200px のため境界直前に始まり、`putImageData` が数十 MB のピクセルバッファ (画質 high = 2×DPR オーバーサンプルで A4 20〜30MB 級) を renderer メインスレッドで同期コピー — 初見ページを下へ読み進めるとき毎回発生 (B) `_currentPage` が切り替わった瞬間の `onPageChange` → `highlightCurrentThumb` の `scrollIntoView` (同期レイアウト強制 + サイドバー側サムネ遅延 render の IPC 連鎖) — 既読ページ間の跨ぎでも毎回発生。**修正 3 点 (すべて表示専用経路 — exporter / compositePage / 回転系 / 印刷は 1 バイト不変)**: ①`_ensureRendered` (viewer.js) の `putImageData` → **`createImageBitmap` + `drawImage`** (デコードは別スレッド・描画は GPU、回転ページの offscreen バウンスも解消。await 追加に伴い「ページ div が差し替わっていたら stale 結果を捨てる」ガード追加 — 捨てないと開き直し後の canvasEls に幽霊 canvas が残り空白ページになる) ②`ROOT_MARGIN` 200px→**1600px** (高 zoom の A4 ≈1300px でも 1 ページ強先に render 開始。canvas は破棄しない設計なので定常メモリ不変) ③`highlightCurrentThumb` (sidebar-thumbs.js) の scrollIntoView を **150ms debounce** (ハイライト自体は即時、追従スクロールだけ連続切替が落ち着いてから 1 回。タブ復帰 / F4 の中央寄せは switchSidebarTab 等が直接 scrollIntoView する別経路 = 不変。v2.0.23 の TDZ 事故を踏まえ timer 変数は関数より前に宣言)。npm test 全スイート fail 0 (renderer-load smoke 含む)。**実機確認チェックリスト**: ①初見の長い PDF を下へ連続スクロール → 跨ぎのカクつき減 (本丸) ②既読ページ間の上下往復 ③スクロール停止後にサムネハイライトが追従 (ワンテンポ遅れて動くのが新仕様) ④回帰: 回転ページの表示向き / ズーム・画質変更後の再描画 / 罫線抑制 ON 表示 / サムネタブ復帰・F4 の中央寄せ (従来どおり即時) / 別 PDF を開き直した直後に空白ページが出ないか ⑤印刷・保存・FAX は経路不変だが普段の操作で各 1 回
- **v2.0.24 (stable、3 OS)** = **v2.0.23 の全 UI 無反応 (「ファイルが開かない」) の緊急 patch** (fix `a77a60b`・release `98088ec`、2026-08-01、**実機 OK 同日**)。**根因**: beta.5 (`1522b15`) が sidebar-thumbs.js のモジュールトップレベルに追加した `let ctxThumbSelection = sidebarThumbSelection;` (88 行目) が、ファイル末尾 1073 行目で宣言される `export const` を **TDZ 中に参照 → ReferenceError → renderer の単一 module graph (index.html → renderer.js のみ) 全体が評価失敗**。窓は出るが全操作無反応 (3 OS とも)。98 風更新ダイアログも renderer 表示のため **autoUpdater でも自己復旧不能** (ヘルプ > 更新を確認も無反応に見える) — 復旧は installer の手動実行。ブラウザ DL がセキュリティで弾かれる場合は WSL から `gh release download` で Downloads に直置き + latest.yml の sha512 照合が有効だった。**修正** = `null` 初期化 + dispatch 側 `?? sidebarThumbSelection` fallback の最小変更 (関数実行時なら宣言済みで TDZ 無し。beta.5 の機能 = 分割側複数選択の右クリック 保存/印刷/削除 はそのまま)。**すり抜けた構造的原因 → 再発防止**: ①**CI に renderer をロードするテストが皆無だった** (node --test 系は関数単位 import、test:m1 は main プロセス専用ランナー) → renderer.js 末尾に module-load sentinel (`window.__rendererLoaded`) を追加し、実 BrowserWindow で本番同一 webPreferences の index.html をロードして graph 完走を assert する **`test/renderer-load-smoke.mjs` を electron-runner 先頭に新設** (導入時にバグあり状態で本番と同一の ReferenceError を捕捉 → 修正後にパス、の有効性検証済み。以後この事故クラスは push ごとに 3 OS CI で自動検知)。②beta.5/6 が実機未走行のまま当日昇格し、**実機初走行 = stable 更新の瞬間**になった → **運用ルール: β は最低 1 回実機で触ってから昇格する** (PDF を 1 つ開くだけの 1 分確認で防げた)。memory [[v2023-beta-status]]
- **v2.0.23 (stable、3 OS — ⚠️ 欠陥版、v2.0.24 で置換)** = **β1〜6 の昇格** (2026-08-01)。内訳: beta.1=確定保存後のしおり自動取込 / beta.2=Electron 42.7.1 + better-sqlite3 12.11.1 / beta.3=回転ターゲット修正 / beta.4=サムネ右クリック印刷 / beta.5=分割サムネ右クリック複数選択 / beta.6=回転吹き出し誤軸フィット。Mac/Linux へは本 stable で β1〜6 の内容が初配布 (Electron 42 化含む — Mac は自前アップデータ、Linux は deb 自動更新)。`releaseType` を `prerelease`→`release` に戻した (次の β で再度 `prerelease` へ)。
- **v2.0.23-beta.6** = **回転済み吹き出しの自動フィット誤軸スナップを先回り修正** (fix `5a70e1d`・release `949b149`、2026-08-01 — 実害報告前のユーザー依頼「不具合が出ないうちに解消したい」)。v2.0.13-beta.3 で記録した未対応エッジ。**契約** (viewer 回転 textNode / exporter drawOverlay): rotation 90/270 の overlay は canonical w/h が回転後の外枠、本文は回転前 (natural) frame = 縦横入替で折り返す。ところが overlay-edit.js の自動フィット 3 経路が全部直立前提で canonical w を折返し幅に使っていた — ①`handleOverlayResizeEnd` (リサイズ後スナップが誤軸) ②`fitCalloutBox` (options bar のフォント/サイズ変更後の自動拡大が誤軸) ③`handleTextEditCommit` (直立エディタの可視サイズを転置せず保存 = 縦横転置。**調査中に判明した同族の未記録バグで、回転済みテキスト枠にも同じ穴 — 同時修正**)。修正 = isVert (90/270) のときだけ natural frame で採寸→canonical に転置する追加分岐。**rot=0/180 は従来式と同値で不変** (回帰テストで固定)。描画側 (viewer/exporter) は v2.0.13-beta.3 で修正済みのため不変更。`test/callout-rotation-fit.test.mjs` 10 件新設 (stub canvas は text-layout-measure と同じ規約)、npm test 全 13 スイート fail 0。実機で見る場合: ページ 90° 回転で吹き出し本文が縦に流れる状態を作り、リサイズ縮小→正しい向きで最小に戻る / フォントサイズ変更→本文方向に拡大 / 本文編集確定→枠が転置しない、＋無回転の吹き出し回帰。
- **v2.0.23-beta.5** = **分割サムネ右クリックの 保存/印刷/削除 が分割側の複数選択を拾うように** (fix `1522b15`・release `1fe46a0`、2026-08-01)。⚠️ **この fix にモジュールレベル TDZ 参照が混入しており、実機未検証のまま stable 昇格した結果 v2.0.23 が全 UI 無反応になった (v2.0.24 エントリ参照)**。機能自体は v2.0.24 で健在。beta.4 の既知仕様 (複数選択判定がサイドバー選択のみ) を、ユーザー要望「左サムネも分割サムネも同じ操作感を期待している。メニューがあるのに機能が異なるのは違和感」で解消。右クリックメニュー自体はサイドバー/分割で共有のまま、`attachThumbContextMenu` に選択セット引数を追加 (**default = sidebar で従来経路は不変**)、split-view 側の呼び出しだけ `splitThumbSelection` を渡す追加分岐。メニューを開いた列の選択で 保存/印刷/削除 のラベルと対象を決める (Del キーが既に per-context な `deleteSelectedPages(state)` と同じ型)。セマンティクスは左サムネと同一 (右クリックページが選択に含まれれば選択全体 / 含まれなければ 1 ページ)。印刷本体・`actionSavePagesAsPdf`・Adobe `/p` 経路は不変。npm test 全 12 スイート fail 0。
- **v2.0.23-beta.4** = **サムネ右クリックに「このページを印刷…/選択した N ページを印刷…」を追加** (feat `bb527ec`・release `ce35201`、2026-07-27、Windows β 配信 — **実機確認済み 2026-08-01 OK → クローズ**)。ユーザー要望「選択ページの PDF 保存に加えて印刷も右クリックから」。メニューは「PDF として保存…」の直下、**save-page と同じ複数選択セマンティクス** (右クリックページがアクティブな複数選択に含まれるなら選択全体 / それ以外はクリックした 1 ページ — プレーンクリック 1 ページや選択外ページの単独印刷がこれで確実にできる。ツールバー印刷の従来絞り込み = 2 ページ以上 or Ctrl/Shift 明示 1 ページ (v2.0.14 の `_sidebarSelectionUsable`) は不変)。実装 = `actionPrint({forcePages})` を print-flow.js に**追加分岐** (指定時は split/sidebar 選択の推測をスキップして明示リストを preselected に採用 — 右クリックは選択状態を変えないため推測では表現できない)、以降は既存 Adobe `/p` 経路 (案 D) / legacy ダイアログにそのまま流れる = **印刷本体・白黒・FAX・下敷きは 1 バイトも不変**。配線は S6 callback 注入 (initSidebarThumbs に actionPrint)。⚠️ 既知の仕様: split-view サムネの右クリックでも同メニューが出るが、複数選択判定は**サイドバー選択のみ**参照 (既存の保存/削除項目と同じ) — split の複数選択印刷は従来どおりツールバーで (→ **beta.5 で解消** — 分割側の複数選択を拾うようになった)。npm test 全 12 スイート fail 0。**実機確認チェックリスト**: ①右クリック「このページを印刷…」→ Adobe ダイアログで対象がそのページのみ ②Ctrl/Shift 複数選択 → 選択内サムネ右クリック → ラベル「選択した N ページを印刷…」で N ページ対象 ③複数選択がある状態で**選択外**サムネを右クリック → クリックページのみ ④回帰: ツールバー印刷/白黒印刷/FAX/下敷きが従来どおり
- **v2.0.23-beta.3** = **最終ページを左サムネで選択して回転すると 1 つ前のページが回転するバグの修正** (fix `cbe1176`・release `710e8c7`、2026-07-27、Windows β 配信 — **実機確認済み 2026-08-01 OK → クローズ**)。**根因**: サムネのプレーンクリックは選択 + `scrollToPage` するが、最終ページが viewport より背が低いと**スクロールが最下部で clamp されページ先頭に届かない**。単一選択は β8/β9 対策 (stale 選択で関係ないページが回転) で意図的に無視され `visiblePageNow()` = **viewport 最上部のページ**に落ちるため、最上部に残っている 1 つ前のページが回転していた。縮小表示 (全ページが 1 画面に収まる等) では最終ページ以外でも起きる (常に p.1 が回る)。**修正** = `resolveRotationTargets` (renderer.js) に**追加分岐**: 単一のサイドバー選択でも「選択ページが今 viewport 内に見えている」なら回転対象に採用 (`viewer.isPageVisibleNow(pageNo)` を viewer.js に**新設** — visiblePageNow と同じ同期 scrollTop 読み)。画面外の stale 選択は従来どおり無視 = β8/β9 挙動維持、**`rotatePageBy` 本体・複数選択・split-view・右クリック回転は 1 バイトも不変** (回転系不動ルール遵守)。⚠️ §8.2 🔴(2)「一括回転で回転自体が効かない」(再現待ち) とは**別問題** — あちらは rotatePageBy 冒頭の find 失敗による無言 skip 疑い、今回はターゲット解決層の選定ミス。npm test 全 12 スイート fail 0。**実機確認チェックリスト**: ①最終ページをプレーンクリック → ↺/↻ → 最終ページだけが回転 ②縮小表示でサムネクリック → 回転 → クリックしたページが回る ③回帰: サムネクリック後に別ページへ大きくスクロール → 回転 → **見ているページ**が回る ④回帰: 複数選択一括回転 / split-view 選択回転 / サムネ右クリック回転
- **v2.0.23-beta.2** = **Electron 41.7.1→`42.7.1` + better-sqlite3 12.10.0→`12.11.1` の単独載せ替え β** (chore `7c17123`・release `daad609`、2026-07-24、Windows β 配信 — **実機確認済み 2026-08-01 OK → クローズ**)。Electron 41 EOL (2026-08-25) 対応、ADR-0004 の解除条件充足 (better-sqlite3 v12.11.1 が **electron-v146 prebuild を win32-x64/darwin-x64/darwin-arm64/linux-x64 の全 4 つで提供** — release assets を gh api で確認済み。Windows ビルドエラー修正 PR #1488 収録、issue #1474/#1475 closed)。42.7.1 = 42 系最新 (CVE-2026-34781 clipboard DoS / CVE-2026-34769 commandLineSwitches injection とも修正済み版数)、better-sqlite3 13.x は major のため見送り (単独載せ替えの切り分け優先)。いずれも exact 固定、ADR-0004 に 2026-07-24 更新節を追記。**旧 `security/electron-42-upgrade` ブランチ (v2.0.11 分岐・42.5.2) はマージせず main へ載せ直しで supersede — ブランチは破棄してよい**。**42 化完了により月次監視 routine (`trig_014hyr1dE1yVZZWf23J1PRZN`) は役割終了 — 停止してよい**。npm test 全 pass (全スイート fail 0、skip 1 は既存)・CI test.yml (3 OS、実 Electron 42 起動 = test:m1) 緑・release ビルド一発成功。Electron 42 のサポート期限 〜2026-10-20。**実機確認チェックリスト (42 = Chromium 更新の影響を見る)**: ①beta.1→beta.2 のアプリ内更新が当たる (autoUpdater 回帰) ②既存 workspace が開く (better-sqlite3 native) ③**テキスト overlay の折返し・位置が v2.0.22 と同一** (Chromium のフォントメトリクス変化領域 — ADR-0028 の canvas 採寸なら一致するはずだが本 β の本丸) ④blob 画像 (サムネ/ページ描画) ⑤Adobe `/p` 印刷 1 回 ⑥98 風ダイアログの表示崩れなし ⑦しおり (beta.1 修正) 引き続き正常。問題なければ v2.0.23 stable 昇格
- **v2.0.23-beta.1** = **確定保存 (上書き) 後にしおりを追加すると既存しおりが全部消えたように見えるバグの修正** (実装 `d95915e`、2026-07-24、Windows β 配信 — **実機確認済み 2026-07-24 OK → クローズ**)。症状: ページ挿入 → 上書き (確定) 保存 → しおり追加で、それまでのしおりが消え新規 1 件だけ表示 (新規を消すと復活・アプリ再起動で正常化 — ユーザー実機報告と全一致)。**根因**: しおりペインは「workspace しおりが 1 件でもあればそれを表示 / 0 件なら元 PDF の /Outlines を読み取り専用フォールバック表示」の二層構造 (bookmark-pane.js `refreshBookmarks`) だが、**/Outlines → workspace の自動取込が `openPdfPath` (「開く」操作) にしか無く、確定保存後の新フラット workspace への切替 (save-flow.js が `kpdf3.openPdfFile` を直接呼ぶ ADR-0026 経路) では走らない**。確定直後はしおりテーブルが空 → 書き戻された /Outlines がフォールバック表示 (「(元 PDF / 編集不可)」ラベル付きだが見た目はほぼ従来どおりで気づかない) → 1 件追加した瞬間に workspace 表示 (新規のみ) へ切替 = 「全部消えた」。再起動で直るのは開き直しが openPdfPath 経由で自動取込が走るため。**潜在バグも同時解消**: この状態のまま再確定すると workspace しおり 0 件で /Outlines write-back (main.js `export-pdf-rasterized`) がスキップされ、**ファイルからしおりが全消失**するところだった。**修正** = 自動取込ガードを `autoImportOutlinesIfEmpty()` (bookmark-pane.js) に共通化 (0 件かつ /Outlines ありのときだけ取込、1 件でもあれば no-op = 再取込によるユーザー編集の複製/破壊なし) し、openPdfPath の既存インラインを置換 (挙動同一) + save-flow.js の確定後切替直後 (`_refreshViewer` 後) に呼び出しを追加。byte-copy (無編集上書き = 同一 workspace 再利用でしおり生存) は自動 no-op、別名保存は `newTabAndOpen` → openPdfPath 経由で従来から取込済みのため無関係。npm test 全 pass (181 pass / fail 0、skip 1 は既存)。実機確認項目 (2026-07-24 に確認済み): ①しおり入り PDF → ページ挿入 → 上書き確定 → しおりタブに「(元 PDF / 編集不可)」ラベルが**出ない**こと → しおり追加で既存＋新規が並ぶ ②新規削除で既存がそのまま残る ③再編集 → 再確定 → Adobe で開いてしおり全件生存 (潜在バグ側) ④回帰: 普通に開いた時の自動取込 / 別名保存 (新タブ) でしおりが見える / しおり無し PDF の確定保存
- **v2.0.22 (stable、3 OS)** = **ADR-0028「テキスト表示レイアウトの正を canvas 採寸に統一」(案 C) + paste 正規化** (2026-07-24。β `v2.0.22-beta.1`/`beta.2` の実機確認を経て同日昇格 `cc0e992`、3 OS 一発成功)。**(1) 案 C 本体 (`7fcdd88`)** — マルチディスプレイのテキスト表示ずれ根治 (§8.2 ✅ 参照)。非編集時の text (回転含む) / form_field(text) の viewer 表示を、DOM 自動折返しから **exporter と同一の canvas 採寸 (`wrapCanvasText` @ EXPORT_ZOOM 固定) による「1 行=1 span」絶対配置**に変更 — どのモニタ幅/DPR でも折返し・行位置が確定出力 (真の位置) と一致する。exporter.js に共有採寸 `measureOverlayTextLayout(ov, ctx?)` を**追加のみ** (既存出力経路は 1 バイトも不変更、ctx 注入で node テスト可)。DOM ベースライン合わせ = span の line-height を fontBoundingBox ascent+descent 実測に固定 (half-leading 0) + top = baseline×zoom − ascent (`_domLineMetrics`、viewer.js)。text overlay の画面クリップを exporter と同じ**枠+8pt** (`clip-path: inset(−8pt×zoom)`) に統一。編集中は contentEditable 維持 (IME 制約)、編集キャンセル (Esc) は textContent 復元 → ページ再描画に変更。**対象外 (非干渉): stamp / callout 本文は従来 DOM のまま**。挙動変化 (ユーザー許容済): 編集終了時の文字スナップ / リサイズ中のライブ再折返し廃止 (確定時に再折返し) / 禁則なし折返し。ADR-0002 の layout-engine 部分を supersede (mupdf-layout.js は復活させない)。`test/text-layout-measure.test.mjs` 6 件 (stub ctx で wrap/align/baseline/回転 natural frame/空行スキップを固定) を npm test に追加。**(2) beta.2 = paste 正規化 (`3c604f7`)** — β.1 実機報告「テキストを繰り返し貼り付けて確定すると貼り付けごとに全部改行される」の根治。**根因 = 行単位コピー (メモ帳/Word 等) の text/plain は末尾改行付き「テキスト\r\n」で、contentEditable の default paste に任せると貼り付けごとに `<div>` 分割された DOM になり、コミットの innerText 抽出で 1 貼り付け = 1〜2 個の改行が本文データに焼き込まれていた** (v2.0.21 以前からの潜在挙動。合成再現 = Electron `clipboard` + `webContents.paste()`、WSLg で実行可)。修正 = `enterTextEdit` に paste ハンドラ (finish で解除): 常に text/plain を正規化して挿入 — CRLF/CR→LF、**末尾の改行のみ除去 (文中の改行 = 複数行貼り付けは保持)**、リッチ書式 (コピー元 font-size span 等) 排除、テキスト無し clipboard (画像等) は無挿入。テキスト/吹き出し/スタンプ/フォーム欄のインライン編集全部に効く。**実機確認済 (2026-07-24)**: マルチディスプレイ表示一致・paste 連続貼り付けの改行解消とも OK。⚠️ 既に改行が焼き込まれた既存 overlay は自動では直らない (再編集で削除)。memory [[project_text_layout_unification_plan]]
- **v2.0.21 (stable、3 OS)** = **S6 リファクタ (REVIEW-2026-07 #8) 完了 — renderer.js を 6 モジュールに分割、純移動・機能変更ゼロ** (2026-07-23。β `v2.0.21-beta.1` で実機一巡 → 同日 stable 昇格。**これで 2026-07 全体レビュー 11 件は全完了**)。renderer.js 8,050 → 4,997 行 (B2 完了時 4,472 行にほぼ復帰)。**(その1)** `image-export.js` 新設 (β97 画像書き出し 2 機能: `actionExportAsImage`/`startRegionImageDrag`/`saveRegionImage`/`showImageExportDialog`、~390 行) **(その2)** `save-flow.js` 新設 (確定/上書き/別名の共通パイプライン + ADR-0026 戻す: `actionSave`/`actionExportToPath`/`actionRestoreEditableMaster`/`refreshRestoreMasterUI`/`isPdfOutOfSync`/`refreshDocStateField`、~420 行) **(その3)** `sidebar-thumbs.js` 新設 (最大の塊 ~1,280 行: サムネ描画 observer 遅延 `rebuildThumbs`/`renderThumb`、D&D 並び替え + 外部ファイル/別ウインドウ挿入、＋gap と白紙挿入ダイアログ、複数選択 `sidebarThumbSelection`/`splitThumbSelection`、削除経路 `deleteSelectedPages`、右クリックメニュー `actionSavePagesAsPdf` 含む、サイドバー開閉/タブ切替。`rotatePageBy` は callback 注入で回転系本体は不動、`currentSidebarTab` はタブ snapshot 連携のため get/set export = §4.4 パターン 4、split-save パネルは selection/D&D/gap ヘルパーを import 共有) **(その4)** `split-view.js` 新設 (~575 行: 分割保存パネル `actionSplitSave`/`setSplitMode`/分割確定/`refreshSplitView` + β.123 プログレッシブサムネ生成 + 日付プレフィックス/サムネサイズ UI。sidebar-thumbs.js の selection/D&D/gap/コンテキストメニューを import 共有、`isSplitMode` は `isSplitModeActive()` で export し print-flow / sidebar-thumbs への注入は従来どおり renderer 経由 = 両モジュール無変更、`splitState` は export して renderer の thumbCache invalidate / `rotatePageBy` 経路は不変。renderer 側の死に import 除去も同 commit) **(その5)** `page-numbers.js` 新設 (`1bea32a`、~200 行: ＋ページ番号一括 `openPageNumbersDialog`/`applyPageNumbers` + β.119 プリセット永続化。isOpen/projectStore/history/pendingDeletedPages を getter 注入。移動で未使用化した `getTextFontStack`/`measureTextOverlaySize` の import も同 commit で除去) **(その6)** `form-tab-order.js` 新設 (`a1a2b74`、~465 行: β.82 B-6 Tab 順編集モード一式 — `setTabOrderEditMode`/番号バッジ + MutationObserver/バッジ D&D/リスト popup の並べ替え・クリック強調・位置永続化。`tabOrderEditMode` は state ごと移動し `isTabOrderEditMode()` で公開 = §4.4 パターン 3、記入モードとの排他は `setFormFillMode` callback 注入で双方向維持、projectStore 購読からの popup 再描画は `refreshTabOrderListPopupIfOpen()` に集約、トグルボタン配線もモジュール側へ) **(+cleanup)** `9e09cd9` で exporter 未使用 import 3 件 (`composePageImage`/`composeRegionImage`/`byteCopyEligible`、その1/その2 時点からの残置) を除去。**いずれも純移動・機能変更ゼロ** (§4.4 パターン 1 の getter 注入。`workspaceMutated` 書込は setter 注入、menuBar は `setMenuEnabled` callback — early boot の TDZ throw は従来どおり catch)。各 commit で npm test 全緑 (11 スイート fail 0)。回転系 (`rotatePageBy` 周辺) は完成領域のため**不動のまま** (callback 注入経由のみ)。**実機一巡は v2.0.21-beta.1 (Windows β、`5375dce`) で実施 — ユーザー所感「おおむね OK。厳密にはすべてチェックできていないが前に進める」で stable 昇格を判断** (`3e46a6f`、3 OS 一発成功)。⚠️ チェックリスト全項目の厳密網羅はしていないため、S6 で動かした箇所 (サムネ/分割/保存/画像書き出し/ページ番号/Tab 順) で今後不審な挙動が報告されたら、まず「純移動の取りこぼし (getter 注入漏れ・配線漏れ) = v2.0.20 との差分」を疑うこと。**着手動機 = 下記テキスト表示ずれ (§8.2 🆕) の根治 (案 C) を「次の大物」と位置づけたため — S6 完了により案 C (ADR-0002 supersede ADR 起草) に着手可能**。memory [[project_s6_refactor_progress]]
- **v2.0.20** = **Mac の FAX 送信ルート成立 — プレビュー.app 手動送信 (案 A-2、2026-07-15、実機 OK)**。Mac の FAX ボタン = 白黒組み立て PDF (`assembleHybridPdf`) を temp に書き `open -a Preview` でプレビュー.app に渡す (新 IPC `kpdf3:fax-open-preview-mac`) → ユーザーが ⌘P → FAX キュー選択 → プリント → **宛先ダイアログはプリント後にポップアップ** (Windows と同じ後出し方式、FF Direct Fax Driver) → K-PDF3 は「送信完了」明示確認モーダル (β.129 と同思想、完了自動検出は Mac でも構造的に不可能)。案内にはプリンタ表示名を使用。⌘P の自動化は**プレビューが AppleScript 非対応**で GUI スクリプティング (アクセシビリティ権限必須 + 脆) しか無く却下。Windows/Linux 経路は不変。⚠️ **最重要の教訓: 宛先なしジョブを FAX キューに流し込むと FF ドライバの宛先ヘルパーが壊れ、以後どのアプリからも宛先ダイアログが出なくなる** (18/19 の試行で実発生。キュー掃除・再起動でも直らず**ドライバ再インストールで復旧**) — Mac で FAX キューへ silent 送信する実装は絶対にしない。付随知見: プリンタ情報の「場所」欄空白は正常 (CUPS location は任意メタデータ)。残確認 2 点 = プレビューのプリント設定「実際のサイズ」で実寸照合 / ドライバのアドレス帳・履歴オフ運用。memory [[project_mac_fax_route]]
- **v2.0.19** = **(Mac FAX 試行 2/2 — 経路は直したが Chromium 案自体が却下に)** FAX 判定を表示名でも照合 (Mac は CUPS キュー ID = 装置名が**変更不可・fax 文字なし**のことがあり、β.89 最終チェック警告が毎回出て main も CUPS 直送に流れていた) + FAX ボタン経路に `faxHint:true` (print-pdf-silent / silentPrintPdf の isFax 判定に OR、既定 false で Windows 不変)。これで silent:false 経路には到達したが、**macOS の `webContents.print({silent:false}) は印刷ダイアログを出さず既定設定で即時投入することが実機確定**(宛先なしジョブがキューに入るだけ)。printWindow の PDF viewer も白紙表示 = 白紙送信リスク。**→ Chromium 経由の Mac FAX/印刷ダイアログ案は全面却下** (β72 の Linux teardown crash 却下歴に「Mac はダイアログ不発」が加わった)。faxHint と表示名照合の修正自体は正しく v2.0.20 でも現役
- **v2.0.18** = **(Mac FAX 試行 1/2)** Mac の FAX ボタンが「No PDF Reader detected」で必ず落ちる問題 (pdf-reader-finder は win32 専用) に darwin 分岐を追加し既存 Chromium silent:false 経路へ→ FAX 判定が装置名のみで CUPS 直送に流れ不発。「Adobe 経由」UI は Mac 非表示化 (これは現役)
...
renderer.js は中央 orchestrator として残し、機能別モジュールに分散 (B2 完了時 4,472 行 → 2026-07-10 実測 8,019 行に再肥大 → **S6 リファクタ = REVIEW-2026-07 #8 で 2026-07-23 にその1〜その6 + cleanup 完了、4,997 行**。v2.0.21 stable で配信済み):
...
| `image-export.js` | 390 | β97 画像書き出し (PDF→画像 / 範囲選択画像保存) — S6 その1 (2026-07-23) |
| `save-flow.js` | 423 | actionSave / actionExportToPath / ADR-0026 戻す + 確定版ステータス表示 — S6 その2 (2026-07-23) |
| `sidebar-thumbs.js` | 1367 | サムネ描画 (observer 遅延) + D&D 並び替え/挿入 + ＋gap/白紙挿入 + 複数選択 + 削除 + 右クリックメニュー + サイドバー開閉/タブ切替 — S6 その3 (2026-07-23)。selection/D&D/gap ヘルパーは split-view.js と共有 |
| `split-view.js` | 575 | 分割保存パネル (actionSplitSave / 分割点 UI / パート名 / 日付プレフィックス / β.123 プログレッシブサムネ / 分割確定 / refreshSplitView) — S6 その4 (2026-07-23)。isSplitMode は isSplitModeActive() で export、splitState も export (renderer の invalidate 経路用) |
| `page-numbers.js` | 226 | ＋ページ番号一括 (openPageNumbersDialog / applyPageNumbers + β.119 プリセット永続化) — S6 その5 (2026-07-23) |
| `form-tab-order.js` | 540 | β.82 B-6 Tab 順編集モード (番号バッジ + MutationObserver / バッジ D&D / リスト popup 並べ替え・強調・位置永続化) — S6 その6 (2026-07-23)。tabOrderEditMode は state ごと所有 (パターン 3)、isTabOrderEditMode() で公開。記入モードとの排他は setFormFillMode callback 注入 |

renderer.js に残る責務: `isOpen` / `projectStore` / `history` / `viewer` 等の core state、`refreshViewer` / `actionOpen` / `openPdfPath` / `actionExport` (別名ダイアログ→save-flow 委譲)、search、回転系 (`rotatePageBy` — 完成領域)、各種メニュー glue。**S6 は完了 (v2.0.21 で配信済み)**。
...
| `domain/page-registry.js` | virtualization 用 page metrics cache + 縦レイアウト + visiblePageRange + `currentPagePos` (現在ページ判定: 上端 + 高さ × 1/3 の点 + 両端規則、v2.0.27) |
...
| 分割保存 | toolbar「分割保存」→ サムネビュー + 区切りクリック + 各パート命名 + 日付プレフィックス。分割サムネは Ctrl/Shift click 複数選択 + **Ctrl+A 全選択 (v2.0.27)** + 右クリック 保存/印刷/削除 |
...
| 印影黒印刷 (旧「白黒印刷」、v2.0.27 で改名) | toolbar「印影黒印刷」/ 印刷メニュー → **この 1 回だけ** 書き込み overlay (テキスト / スタンプ / 印影 / 図形 / 吹き出し / フォーム枠) を黒に投影して通常印刷フローへ (`actionPrint({mono:true})`)。元 PDF の中身・マーカー (蛍光)・プリンタ設定は不変。書き込みが無い PDF では「印刷」と同一。用途 = 白黒プリンタで赤印影が薄くなる事故の回避。FAX は常にこれを強制、下敷き印刷は常に色そのまま |
| 文字選択モード (v2.0.26) | toolbar の I ビームアイコン (v2.0.27 でアイコンのみに、ラベル無し。ホバー / » 退避時は「文字選択」) → OCR 済み PDF の文字をなぞって選択 → Ctrl+C / 右クリックでコピー (透明テキストレイヤー方式、`text-select.js`) |
...
| ページ挿入 | サムネ間「+」hover → クリックでダイアログ (白紙 / テキスト付き、72pt 表示)。白紙も外部 PDF 挿入と同じ「見えている隣接ページの中点 display_order」で位置決め (v2.0.27、`afterKey` → `Workspace.visualGapAfter`) |
...
| ページ回転 | toolbar ↺/↻ + サムネ右クリック menu (split / sidebar / viewer の優先順、`visiblePageNow` = `currentPagePos` の同期 scrollTop 読みで race-free)。**v2.0.28-beta.1**: 全経路が `runRotationWithIndicator` を通る — 処理中は「ページを回転」モーダル (段階: PDF に記録中 → 画面とサムネイル更新中 / 何ページ目 / 経過秒 / %)、追加クリックは「回転処理中です」で拒否、「中止して元に戻す」で未処理は回転せず処理済みは逆回転。`rotatePageBy` 本体は不変 (`rotate-progress.js`) |
...
| 現在ページ判定 (v2.0.27) | viewport 上端 + 高さ × 1/3 の点を含むページ (`CURRENT_PAGE_PROBE`)。文書先頭が見えていれば 1 ページ目・末尾が見えていれば最終ページ。`scrollToPage` は行き先を `_navPin` で固定し手 scroll で解除。しおり / 回転 / 分割自動選択 / 印刷「現在のページ」/ N/M / サムネ枠すべて同規則。**v2.0.28-beta.1**: `setZoom` は pin が有効なら比率スクロールではなく pin ページ先頭に揃える (回転直後の fit 再適用で隣ページが current に落ちるのを防ぐ)。fit の自動再適用は viewport 寸法が実際に変わったときだけ (`fit-guard.js`) |
...
| `renderer-load-smoke.mjs` (v2.0.24、renderer module graph が評価完了するか) | 2 pass | Electron runner (先頭) |
...
| `inserted-page-order.test.mjs` (v2.0.27-beta.5 白紙位置) | 12 pass | Electron runner |
| `stamp-export-import.test.mjs` (userData 差し替えのため最後) | 13 pass | Electron runner |
...
⚠️ **Electron ランナーの落とし穴 (2026-08-29 発見・v2.0.27-beta.6 で修正)**: 先頭の `renderer-load-smoke.mjs` が BrowserWindow を `destroy()` すると Electron 既定の `window-all-closed` → quit (exit 0) が走り、**v2.0.24 から約 1 か月、smoke 以降の 6 本が `npm test` で一度も走らず緑のままだった**。`test/electron-runner.cjs` に `app.on("window-all-closed", noop)` を入れて修正。**npm test が緑でも、Electron ランナーの出力に「=== Result」/「N pass」行が 8 本ぶんあるかを確認すること** (npm 経由だと stdout が途中で切れることもあるので、疑わしいときは `./node_modules/.bin/electron --no-sandbox test/electron-runner.cjs > out.txt 2>&1` で直接回す)。
...
git log --oneline | head -20       # 最新は v2.0.27-beta.2 (Windows β、2026-08-22、CI 成功・配布物確認済み)。stable は v2.0.26 (2026-08-20、3 OS)
npm test                           # node --test 16 スイート + electron-runner、全 fail 0 (2026-08-22 実測)
...
**症状**: ページ数の多い PDF の途中ページを回転すると画面がちかちかしてフリーズ。ユーザーは PDF を縦回転モニターの全画面で見ている (2026-09-10 確認) = 成立条件そのもの。

**根因 (実アプリ + CDP トレースで確定、冒頭 v2.0.28-beta.1 エントリ参照)**: fit-width の `ResizeObserver` 再適用 × `setZoom` の比率スクロール × 現在ページ判定 1/3 probe の三つ巴で、回転した横長ページと隣の縦長ページの間でズーム 0.943 ↔ 1.334 を 0.5 秒周期で無限往復。1 周ごとに全ページ再構築 + 重い描画 → main / renderer とも 100%。

**対策 (fix `fc6042f`)**: `fit-guard.js` (幅が変わったときだけ再フィット + 往復検知クールダウン) / `viewer.setZoom` の pin 維持 / `rotate-progress.js` + `runRotationWithIndicator` (再入ガード・段階/経過秒/% モーダル・中止して元に戻す)。`rotatePageBy` / `resolveRotationTargets` / `rotate-place.js` は不変。

**実機確認 (Windows、縦回転モニター全画面)**:
1. 300 ページ級の途中ページを ↻ → ズームが 1 回だけ変わって止まる (ちかちかしない)、現在ページ表示が回転したページのまま
2. ダイアログの経過秒が進む。大容量なら「記録中」が数秒続くのが正常
3. 処理中に ↻ を押し直す → ステータスバー「回転処理中です」、最終角度は 90° のまま
4. 「中止して元に戻す」→ 元の向きに戻り「回転を中止しました (1 ページを元に戻しました…)」
5. Ctrl+ホイール / ズーム選択で、直前に移動したページが先頭に揃ったままズームされる (仕様変更点。自由 scroll 後は従来どおり)
6. 通常運用 1 週間で回転・ズーム・fit-page に違和感なし → v2.0.28 stable 昇格 (`releaseType` を `release` に復帰)

**残課題 (未着手、次の候補)**: main `kpdf3:set-page-rotation` (main.js `reopenActiveDoc()`) が DB 1 行更新のたびにソース PDF 全バイト再読込 + mupdf 再パース + 全ページ再列挙。300MB で main 停止 1.0 秒 (Linux tmpfs)、Windows の大容量サイドカーではさらに長い想定。`activePages` の該当行だけ更新する追加分岐で解消できる見込み (render 経路は `activePages` の寸法しか見ない)。

**調査の手順メモ (再利用可)**: 合成 PDF は pdf-lib で生成 (A4 300 ページ、2480×3508 PNG をページごとに別画像にすると 300MB 級 = サイドカー経路)。実アプリを `electron . --remote-debugging-port=9333` で起動し、2 つ目のインスタンス起動で PDF パスを渡す (second-instance → `kpdf3:open-pdf-by-os`)。CDP で `page-num-input` に Enter → `btn-rotate-right` click。事前に `Runtime.enable` した常時接続セッションなら、レンダラーが応答不能でも `console.warn` 通知は届く (`Debugger.pause` は効かなかった)。
...
開発一段落を機に全体レビューを実施し、対応タスク 11 件を専用文書 **`REVIEW-2026-07-TODO.md`** に起票 (本書には二重記載しない)。**進捗 (2026-07-06): 8 件完了** — ✅ #10 ゴミログ削除 / ✅ #6 依存 exact 固定 / ✅ #5 Electron 41 EOL = (a) 一時容認 + 監視継続 (ADR-0004 §更新 2026-07-05) / ✅ #11 PAT 失効 2027-05-11・リマインド routine 設定 / ✅ 🔴 #1 workspace 膨張 → **ADR-0027 + v2.0.12-beta.2「ワークスペースの整理」で解決・実機確認済み (2.9GB 回収)** / ✅ #2 バックアップ → `docs/backup/` バッチ + スケジューラ運用開始・復元リハ済 / ✅ #3 パスワード平文化警告 → **v2.0.12-beta.3 配信済・実運用で確認** / ✅ #4 byte-copy ゲート総当たり → **`byteCopyEligible` 一本化 + テスト 20 件、印刷の末尾ページ削除落ち 2 件を同時修正、v2.0.12-beta.4 配信済** (冒頭 patch 一覧参照)。**さらに 2026-07-10 (v2.0.13 stable 昇格時) に 2 件完了**: ✅ #9 確定版ステータス可視化 → ステータスバー常時表示 (v2.0.13 同乗、ADR-0026 UI 追補) / ✅ #7 ドキュメント乖離修正 + 遡及 ADR 0017〜0025 全 9 本起草 (本書の本更新)。**✅ #8 renderer.js S6 リファクタも 2026-07-23 に完了し 11 件全完了** — その1 `image-export.js` (`97c7243`) + その2 `save-flow.js` (`e86f5ef`) + その3 `sidebar-thumbs.js` (`ee3e03f`、最大の塊 ~1,280 行) + その4 `split-view.js` (`cd6a79d`) + その5 `page-numbers.js` (`1bea32a`) + その6 `form-tab-order.js` (`a1a2b74`) + 残置 import cleanup (`9e09cd9`)。renderer.js 8,050→4,997 行、純移動・毎 commit npm test 全緑。実機一巡は v2.0.21-beta.1 で「おおむね OK」→ **v2.0.21 stable (3 OS) へ昇格・配信済み**。完了記録表も ✅ 済み (`a8a380d`)。**着手順・各項目の目的/根拠/手順/完了条件は同文書の「推奨着手順」表と完了記録表に従う** (同文書は随時更新可、本書は従来通り明示依頼時のみ)。

#### ✅ マルチディスプレイでテキスト挿入位置が数 px ずれて見える — 2026-07-22 → **ADR-0028 (案 C) 実装・v2.0.22 stable で解決・実機確認済 (2026-07-24 クローズ)**

**クローズ済**: 案 C を ADR-0028 として起草・実装 (`7fcdd88`)、v2.0.22-beta.1 → beta.2 (paste 正規化 `3c604f7` — β 実機一巡で発見した「貼り付けごとに改行が本文へ焼き込まれる」潜在バグの修正) → **2026-07-24 に v2.0.22 stable 昇格 (`cc0e992`、3 OS)**。マルチディスプレイの表示一致・paste とも実機確認 OK。実装詳細は冒頭 patch 一覧 v2.0.22 と `docs/adr/0028` が正。以下は当時の検討記録。

**症状 (2026-07-22 ユーザー報告)**: 横長ディスプレイ (最大化) でテキストを挿入 → 下書き保存のまま縦長ディスプレイで表示すると挿入位置が若干ずれて見える。縦長側の表示を基準に微調整して確定保存したら、実は横長側の表示が正しく、意図と違う位置に焼かれた。

**原因 (2026-07-23 分析確定)**: 保存座標 (canonical PDF pt) は表示に依存せず**最初から正しい**。ずれるのは表示側 — テキストの見た目レイアウトは ①横長画面の DOM ②縦長画面の DOM ③確定出力 (exporter) の **3 つの別エンジンが各自再計算**しており、③ = `wrapCanvasText` canvas 採寸 (EXPORT_ZOOM=900/72 固定、exporter.js) が事実上の真の位置。viewer は canonical×zoom の CSS px で DOM に組ませるため (viewer.js `_createOverlayElement`)、**fit-width 既定 (renderer.js `zoomMode`) でモニタ幅により倍率が変わる** + モニタごとの DPR で、Chromium の折返し判定・字送り丸め・ベースラインスナップが数 px 級にずれる。倍率の大きい横長画面が③に近かったのは必然 (量子化誤差が相対的に小さい)。

**重要な経緯の発見**: **ADR-0002 (2026-05-09 採用) がまさに「viewer/pdf 共通レイアウトエンジン (mupdf)」を決定済み**で、ラッパー `src/backend/mupdf-layout.js` も M1 で作られたが、**import ゼロのまま休眠** (IME=contentEditable 制約 §13.1 で立ち消え)。NG リスト入りの却下案ではなく「採用済み ADR の未完部分」。

**決定 (ユーザー承認 2026-07-23)**: 根治 = **案 C: 非編集時のテキスト表示を wrapCanvasText 採寸の「1 行=1 要素」絶対配置に統一** (v2.0.13 ベクターテキスト層の 1行=1op 資産を流用)。編集中は contentEditable 維持 (IME 制約)。**ユーザーが許容済みの制約**: ①編集終了時に文字がスナップして動くことがある ②編集中は従来のずれが残る (位置判断は編集を抜けてから) ③禁則の見え方が編集中/確定表示で変わる (wrapCanvasText は禁則なし) ④既存文書の画面折返しが変わって見える。**順序 = #8 S6 リファクタ完了 (✅ 2026-07-23、v2.0.21) → 次セッションで ADR-0002 の supersede/追補 ADR 起草 (正は mupdf でなく canvas 採寸に変更、mupdf-layout.js は復活させない) → 案 C 実装 → β トライアル**。案 B (確定表示トグル) は C 決定により見送り。**つなぎ運用: 位置の最終確認は印刷プレビュー (exporter 経路 = 真の位置) で行う**。なお今回の誤位置ファイルは ADR-0026「編集に戻す」で復旧可能。memory [[project_text_layout_unification_plan]]
...
- `src/renderer/renderer.js` — 中央 orchestrator (B2 で 4,472 行に圧縮 → 8,019 行に再肥大 → S6 リファクタ = REVIEW-2026-07 #8 で 2026-07-23 に 4,997 行へ)
...
```
k-pdf3/
├── package.json                          # v2.0.27 (stable、releaseType=release — β を切るときは prerelease へ)
├── package-lock.json
├── README.md
├── ROADMAP.md
├── HANDOVER.md                           # 本書 (現役の正)
...
│   └── renderer/
│       ├── index.html
│       ├── renderer.js                   # 4,997 行 (中央 orchestrator、S6 その1〜6 抽出後)
...
│       ├── style.css
│       ├── busy-modal.js
│       ├── fit-guard.js                  # v2.0.28-beta.1 fit 再適用判定 (幅変化のみ) + ズーム往復検知
│       ├── rotate-progress.js            # v2.0.28-beta.1 回転インジケーターの % / 文言 (純関数)
│       ├── dialogs.js
│       ├── file-browser.js
│       ├── overlay-edit.js
...
│       ├── print-flow.js
│       ├── tab-manager.js
│       ├── image-export.js               # S6 その1 (β97 画像書き出し)
│       ├── save-flow.js                  # S6 その2 (確定/上書き/別名 + ADR-0026 戻す)
│       ├── sidebar-thumbs.js             # S6 その3 (サムネ描画/D&D/挿入/削除/複数選択)
│       ├── split-view.js                 # S6 その4 (分割保存パネル)
│       ├── page-numbers.js               # S6 その5 (＋ページ番号一括)
│       ├── form-tab-order.js             # S6 その6 (Tab 順編集モード)
...
- **Electron 版数の固定** (ADR-0004 + 2026-06-03 §更新): **2026-06-03 に Electron 38.8.6 → `41.7.1` (exact) へ更新**。Electron 38 が 2026-03-10 で EOL となりパッチ供給が止まったため、固定継続が「アップグレード起因の脆弱性を避ける」本来目的に反する状態となったため前進。ブロッカーだった better-sqlite3 が **12.10.0** で Electron 39/40/41 prebuild を全 OS 提供したのが解決の鍵 (mupdf は wasm で ABI 非依存)。**Electron 42 はまだ不可** (better-sqlite3 が 42 対応を取り下げ済 PR #1470、upstream issue #1474/#1475 オープン)。41 のサポートは **〜2026-08-25**、それまでに 42 が通れば再度上げる。→ **2026-07-24 に `42.7.1` (exact) + better-sqlite3 `12.11.1` へ更新済 (v2.0.23-beta.2、ADR-0004 の 2026-07-24 更新節)** — 解除条件 (electron-v146 prebuild 全 4 プラットフォーム + issue #1474/#1475 closed) が充足。Electron 42 のサポートは **〜2026-10-20**。
...
- **CI / 依存監視整備 (2026-06-03)**: (a) GitHub Actions ランナーの Node 20 ランタイム撤去 (2026-06-16 で Node24 強制 / 09-16 削除) に追従し、`actions/checkout`・`actions/setup-node` を Node24 対応の最新メジャーへ更新し **commit SHA で固定** (checkout v6.0.3 / setup-node v6.4.0。tag 再ポイント攻撃 [tj-actions/changed-files CVE-2025-30066] 対策、release は `package-manager-cache:false` で no-cache 維持)。アプリのビルド Node は `node-version:22.22.2` 固定のまま不変。(b) **Electron 42 化を月次自動監視する claude.ai routine を作成** (`trig_014hyr1dE1yVZZWf23J1PRZN`、毎月1日 09:05 JST。`gh api` で better-sqlite3 の electron-v146 prebuild + issue #1474/#1475 を判定し、揃えば 42 化 PR を自動準備・マージはしない)。GitHub は `/web-setup` で接続済、テスト実行で動作確認済。**(2026-07-24 追記: Electron 42 化完了 (v2.0.23-beta.2) により本 routine は役割終了 — 停止してよい)**
...
| 0004 | Electron 版固定 → 2026-06-03 に 41.7.1 → **2026-07-24 に 42.7.1 へ更新** (better-sqlite3 12.11.1 の electron-v146 prebuild 提供・41 EOL 脱却、v2.0.23-beta.2、§更新節) | ✅ |
...
- **S6 (renderer.js 分割)**: B2 残置責務。**2026-07-23 に完了 (REVIEW #8、v2.0.21 で配信済み)** — その1 image-export / その2 save-flow / その3 sidebar-thumbs / その4 split-view / その5 page-numbers / その6 form-tab-order + import cleanup (8,050→4,997 行、冒頭 v2.0.21 エントリ参照)
