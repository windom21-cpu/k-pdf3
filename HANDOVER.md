...
| 分割保存 | toolbar「分割保存」→ サムネビュー + 区切りクリック + 各パート命名 + 日付プレフィックス。分割サムネは Ctrl/Shift click 複数選択 + **Ctrl+A 全選択 (v2.0.27-beta.6)** + 右クリック 保存/印刷/削除 |
...
| ページ挿入 | サムネ間「+」hover → クリックでダイアログ (白紙 / テキスト付き、72pt 表示)。白紙も外部 PDF 挿入と同じ「見えている隣接ページの中点 display_order」で位置決め (v2.0.27-beta.5、`afterKey` → `Workspace.visualGapAfter`) |
...
| ページ回転 | toolbar ↺/↻ + サムネ右クリック menu (split / sidebar / viewer の優先順、`visiblePageNow` = `currentPagePos` の同期 scrollTop 読みで race-free) |
...
| 現在ページ判定 (v2.0.27) | viewport 上端 + 高さ × 1/3 の点を含むページ (`CURRENT_PAGE_PROBE`)。文書先頭が見えていれば 1 ページ目・末尾が見えていれば最終ページ。`scrollToPage` は行き先を `_navPin` で固定し手 scroll で解除。しおり / 回転 / 分割自動選択 / 印刷「現在のページ」/ N/M / サムネ枠すべて同規則 |
...
| **合計** | **Electron ランナー 8 本 199 項目 + node 系すべて fail 0（2026-08-29 実測、npm test 全 pass）** | |
...
```
k-pdf3/
├── package.json                          # v2.0.27-beta.2 (β、releaseType=prerelease — 昇格時に release へ)
├── package-lock.json
├── README.md
├── ROADMAP.md
├── HANDOVER.md                           # 本書 (現役の正)
