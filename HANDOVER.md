...
クローン同期メモ: 2026-05-12 に開発リポを Public 化する際 `git filter-branch` で全 commit/tag を rewrite + force push しています。すでに同期済の環境では追加対応不要。古いクローンを掘り起こした場合のみ `git fetch --all --tags --force && git reset --hard origin/main` で再同期。
...
- **β12〜β15** (2026-05-12): β11 残バグ (回転後サムネ白紙化) 修正、画像スタンプ「PDF に試し置き」、4K DPI 対応、PDF 関連付け / 単一インスタンス
- **β16〜β30** (2026-05-12): テスター指摘 13 件 + 試し置き UX 連続改善 (パレット未選択初期 / placement ghost と trial cursor の二重表示抑止 / 試し置きに角ハンドル + 中央ドラッグ)
- **β31〜β35** (2026-05-12): D1〜F2 一括投入 (印刷テキスト解像感 600→900dpi / 外部 PDF 挿入 vector 維持 / 太字を独立軸 chk / 数字 hanko 独立軸 chk / `image-rendering: -webkit-optimize-contrast`)。β31/β32 起動クラッシュ騒動 → autoUpdater「後で」由来と推定、β33 を緊急ロールバック後撤回。同時期に開発リポを Public 化 (force push)。CI を案 B-2 (β タグ = Win 単独) に分割
...
1. **冒頭ヘッダ + 「直近進捗」 + 「当面の残課題」** — 現在地を 1 分で把握
...
7. **§17 ユーザー要望タスクリスト** — 中期タスクの全体像 (大半は完了済の参照用)
...
- **v2.0.0-beta.16〜v2.0.0-beta.70** — 2026-05-12〜05-14 (per-β 詳細は git tag + commit message)。要点は冒頭「直近進捗」セクション参照。テスター指摘 13 件着地、4K DPI 対応、スタンプ試し置き UX 仕上げ、印刷品質改善の長い試行錯誤 (案 M / N / N' / ζ → 全部撤回 → C アプローチ = PDF Reader CLI 委譲を採用)、ウインドウ位置永続化、autoUpdater 起動クラッシュ騒動 (「後で」仮説) → β33 緊急ロールバック撤回、開発リポ Public 化 (force push) + CI 案 B-2 分割。
...
git log --oneline | head -20       # 最新は β71 (B2 + B3 着地)
npm test                           # 380/380 pass
npm run dev                        # electronmon (推奨、自動 reload)
# または npm start                 # 単発起動
```

**現在のフェーズ**: β テスト継続中。**§6.4 β テストフロー** が一次資料。新機能は B3 まで完了したので、当面はテスター指摘 → β.N+1 を回しつつ、annotation read-only proxy / qpdf sanitize / autoUpdater「後で」仮説対応など stable に向けた繰越項目を進める。

**配布インフラ**: 開発リポは Public (`windom21-cpu/k-pdf3`)、autoUpdater フィードは別リポ `windom21-cpu/k-pdf3-releases`。CI は **案 B-2** で β タグ=Win のみ・stable タグ=3 OS (§12.1)。

### 8.2 短期の優先順

#### 🔴 着手検討が必要なオープン項目

1. **「後で」仮説の恒久対応** (autoUpdater UX 改修) — ダイアログから「後で」撤去 or キャンセル時の差分ファイル cleanup。詳細は冒頭「当面の残課題」参照
2. **C3 annotation read-only proxy** — Adobe で押した annotation を viewer 表示。新セッション規模、§15.3 参照
3. **qpdf sanitize** — secure export pipeline (metadata strip / xref rebuild)。`extraResources` 同梱で各 OS バイナリ spawn。新セッション規模
4. **PDF 開閉繰り返しでクラッシュ** (β51 ユーザー報告) — 再現したら `userData/crash.log` 解析

#### 🟡 確認待ち項目（実機テスター側）

- **β71 B3 タブ別ウインドウ機能**: 5 経路 (右クリック / ツールバー / File menu / drag tearout / drag dock-back) の業務での体感。tearout / dock-back 中に画面の見え方や残留ウインドウがおかしくないか
- **β54〜β70 印刷経路**: PDF Reader CLI (Adobe / Foxit / PDF-XChange) 経由で安定したか。Acrobat Pro 環境では 30 秒の体感待ち (β69 タイマ kill) を受容できているか
...
- **Wayland ショートカット** — F5 / Ctrl+R / F12 が Ubuntu Wayland で発火しない。バージョン情報ダイアログのリロード / 開発者ツールボタンで代替可。`project_kpdf3_shortcut_unresolved.md` 参照
- **stable リリース時のクリーンアップ** — β51 診断クラッシュロガー一式の撤去 (詳細は冒頭「当面の残課題」)
...
- `docs/adr/` 全部 — 設計判断の根拠 (タブ実装は ADR-0015)
...
- `src/main/main.js` — IPC surface 全体像 (B3 で per-window state が入っている)
- `src/renderer/renderer.js` — B2 で 4472 行に圧縮済。残置されている主な責務: setOpen / refreshViewer / actionOpen / openPdfPath / split-view / sidebar thumbs / page numbering dialog / search / 各種 menu glue。さらなる切出し候補は split-view + sidebar-thumbs (将来 S6 候補) 程度
...
- ~~**タブ未対応**~~: タブ + マルチウインドウ機能 (B3) は β71 で完了。
...
- ~~**タブ実装**~~: ADR-0015 案 B で実装完了 (β1 でタブ機能、β71 でタブ別ウインドウ D&D)。
...
- ✅ **ADR-0015 (tab / multi-window)**: 案 B (renderer 主体タブ管理 + main は per-window active 維持) で実装。タブ機能 (Phase 1-7) は β1 で完了、タブ別ウインドウ D&D (B3) は β71 で完了。前段の renderer.js モジュール分離 (B2) も β71 で完結。
...
- ~~**`src/renderer/renderer.js` のモジュール分離**~~: B2 で完結 (β71、8631→4472 行 / 12 モジュール分散)。残置責務の追加切出し (split-view + sidebar-thumbs を S6 として括り出す等) は将来候補だが緊急性なし。設計記録は project memory `project_kpdf3_b2_refactor.md`
...
- **テストカバレッジ不足**: 2026-05-10 以降の追加分（ページ削除 / 挿入 / Save As / 検索 / スタンプ管理 / 画像スタンプ / 編集可能しおり / callout / 自動リサイズ / タブ別ウインドウ）は手動確認のみ。Electron runner で round-trip テストを追加すべき。特にスタンプ管理 + assets 周り、B3 の dock-back race 周りは migration / smoke 多め
- **B2 完了後の低優先候補**: overlay クリック UX 改善 — 単一クリック=即編集を「単一=選択、ダブル=編集」に変更 (project memory `project_kpdf3_overlay_click_ux.md`)。低頻度なので β 期間中の優先度低
...
補足: タブ別ウインドウ機能 (B3、§17.10) は β71 で完成したので、ライブ同期版が必要なら「タブを別ウインドウへ分離 → そのウインドウで該当ページを表示」で代替可能。この §17.4 のスナップショット型 popup は引き続き「特定 1 ページだけを軽量に並べたい」用途で残す。
...
### 17.10 タブのウインドウ外 D&D ✅ 完了 (2026-05-15 β71、B3)

タブをウインドウ外に D&D → 別ウインドウになる (Chrome / Firefox / Edge 流)。別ウインドウのタブを元タブバーに重ねたら戻る。起動経路は 5 通り (右クリック「別ウインドウへ移動」/ ツールバー「別窓化」/ ファイル → 別ウインドウで開く... / drag tearout / drag dock-back)。

**実装メモ**: main 側に `windowState: Map<winId, {win, activeTabId, ownedTabIds, tabBarOffset, isPrimary}>` を追加、`activeForEvent(event)` でクロスウインドウ race を遮断。tabHandles は flat なまま (workspace は所有変更だけで destroy/再生成しない)。cross-window で source dragend が不発火する Electron 制約に対応するため、target 側 tab-bar の drop event で main の `activeTabDrag` を引き当てる方式 (`kpdf3:tab-drag-start` / `tab-bar-drop` / `tab-was-docked-away`)。子ウインドウ (`isPrimary=false`) は last-tab-out で自動 close (Chrome 風)、primary は空タブで存続。詳細は `src/main/main.js` + `src/renderer/tab-manager.js` + `src/renderer/renderer.js` の B3 関連セクション。
...
### 17.15 自動アップデート組込み ✅ 完了 (β5, 2026-05-11)

`electron-updater@6.8.5` (exact 固定、MIT) を依存追加 + `src/main/updater.js` で `autoUpdater.checkForUpdates()` 起動 + 4 イベント (`update-available` / `download-progress` / `update-downloaded` / `error`) を IPC で renderer に転送 + 98 風 confirm「ダウンロードしますか?」 → busy modal 進捗 → 「再起動して適用しますか?」フロー。**ヘルプメニュー > 更新を確認...** で手動チェック。`app.isPackaged === false` と `--no-update` フラグで dev/skip。配布 feed は別公開リポ `windom21-cpu/k-pdf3-releases`、CI が `--publish=always` で direct push。

**残課題**: 「後で」仮説の恒久対応 (冒頭「当面の残課題」参照)。macOS の auto-update は code-sign / notarization secrets 整備後に有効化。
