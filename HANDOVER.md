...
24. ✅ **qpdf Mac/Linux バンドル** — **stable 配布対象に Mac/Linux も含めることが 2026-05-25 に確定** (ユーザー判断)。**2026-06-03 に Linux 完了** + **2026-06-05 に Mac 完了 (commit `5a52bbb`)** で 3 OS とも同梱済。Linux: 公式 portable v12.3.2 を `vendor/qpdf/linux/` に bin+lib 同梱。Mac: 事務所 M1 実機で Homebrew qpdf 12.3.2 を `install_name_tool` 手動バンドル (arm64 専用、`@executable_path/../lib` + ad-hoc 署名) して `vendor/qpdf/mac/{bin,lib}/` に配置。`qpdf-sanitize.js findQpdfBinary` は win=flat / mac・linux=bin+lib のレイアウト差対応済、`package.json` の mac/linux `extraResources` 設定済。**残りは stable ビルド時の `extraResources` コピー + dmg 実行確認 (Gatekeeper 子プロセス起動) のみ**
...
2. ✅ **qpdf Mac/Linux バンドル** (stable 残務 #5) — Win 同梱済。**2026-06-03 に Linux 完了** + **2026-06-05 に Mac 完了 (commit `5a52bbb`)**。Mac は事務所 M1 実機で Homebrew qpdf 12.3.2 を `install_name_tool` 手動バンドル (dylibbundler 1.0.5 が病的ループのため) で自己完結化し `vendor/qpdf/mac/{bin,lib}/` に配置 (arm64 専用、`@executable_path/../lib` + ad-hoc 署名、`otool -L` で homebrew 依存ゼロ確認、git 100755)。**コード/ビルド配線は全 OS 完了**。残るは **stable ビルド時の実機確認のみ** = dmg 配布形態での Gatekeeper 子プロセス起動 + Electron spawn 経路のセキュア書き出し (qpdf 単体は確認済)。memory `[[project-qpdf-mac-arm64-pending]]`
