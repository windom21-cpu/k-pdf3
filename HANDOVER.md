...
最終更新: 2026-07-23
現在のバージョン: **v2.0.20 (2026-07-15 リリース、stable / 3 OS)**。**`releaseType` は `release`** (2026-07-14 に v2.0.14 で stable 復帰。β タグ `v*-beta.*` は CI で Windows のみビルド、stable タグは 3 OS)。**2026-07-14 に v2.0.14 → v2.0.17**: 14=β トライアル (別名保存 3 修正) の昇格 + 給紙トレイ + Mac アプリ内更新 / 15=Mac 更新の無言失敗つぶし / 16=Mac 更新の実地検証用 (中身は 15 と同一) / 17=給紙トレイ表示名の日本語化。**2026-07-15 に v2.0.18 → v2.0.20 で Mac の FAX 送信ルートが成立** (プレビュー.app 手動送信、実機 OK — 経緯と却下歴は下記 18/19/20 と §15.6)。**基本は stable 運用** (重大バグは patch 2.0.x、新機能/大物は ADR 起草後 → β トライアル → 昇格)。**次の予定: v2.0.14-beta.2 = Electron 41→42 + better-sqlite3 12.11 の単独載せ替え** (単独 β の方針は不変 — beta.1 が UI 強化になったため番号が 1 つ繰り下がっただけ。`security/electron-42-upgrade` ブランチは v2.0.11 分岐なので main への載せ直しから。Electron 41 EOL=2026-08-25、ADR-0004)。**Mac/Linux を Windows 同等にする残りは Office 挿入/署名など (§15.6。印刷=CUPS 直送・FAX=プレビュー手動で成立済)**。
stable 後の patch / β (要約。full 詳細は git log / `CHANGELOG-history.md`):
- **(未リリース、2026-07-23 main 反映済 `97c7243`+`e86f5ef`+`ee3e03f`+`cd6a79d`)** = **S6 リファクタ (REVIEW-2026-07 #8) — その1〜その4 完了 (抽出フェーズ全完了)**。renderer.js 8,050 → 5,646 行。**(その1)** `image-export.js` 新設 (β97 画像書き出し 2 機能: `actionExportAsImage`/`startRegionImageDrag`/`saveRegionImage`/`showImageExportDialog`、~390 行) **(その2)** `save-flow.js` 新設 (確定/上書き/別名の共通パイプライン + ADR-0026 戻す: `actionSave`/`actionExportToPath`/`actionRestoreEditableMaster`/`refreshRestoreMasterUI`/`isPdfOutOfSync`/`refreshDocStateField`、~420 行) **(その3)** `sidebar-thumbs.js` 新設 (最大の塊 ~1,280 行: サムネ描画 observer 遅延 `rebuildThumbs`/`renderThumb`、D&D 並び替え + 外部ファイル/別ウインドウ挿入、＋gap と白紙挿入ダイアログ、複数選択 `sidebarThumbSelection`/`splitThumbSelection`、削除経路 `deleteSelectedPages`、右クリックメニュー `actionSavePagesAsPdf` 含む、サイドバー開閉/タブ切替。`rotatePageBy` は callback 注入で回転系本体は不動、`currentSidebarTab` はタブ snapshot 連携のため get/set export = §4.4 パターン 4、split-save パネルは selection/D&D/gap ヘルパーを import 共有) **(その4)** `split-view.js` 新設 (~575 行: 分割保存パネル `actionSplitSave`/`setSplitMode`/分割確定/`refreshSplitView` + β.123 プログレッシブサムネ生成 + 日付プレフィックス/サムネサイズ UI。sidebar-thumbs.js の selection/D&D/gap/コンテキストメニューを import 共有、`isSplitMode` は `isSplitModeActive()` で export し print-flow / sidebar-thumbs への注入は従来どおり renderer 経由 = 両モジュール無変更、`splitState` は export して renderer の thumbCache invalidate / `rotatePageBy` 経路は不変。renderer 側の死に import 除去も同 commit)。**いずれも純移動・機能変更ゼロ** (§4.4 パターン 1 の getter 注入。`workspaceMutated` 書込は setter 注入、menuBar は `setMenuEnabled` callback — early boot の TDZ throw は従来どおり catch)。各 commit で npm test 全緑 (11 スイート fail 0)。タグ無し = CI ビルド未発火・業務実機は v2.0.20 のまま。**残り: 実機一巡 (次の実機作業) → (余力・やるかはユーザー判断) ページ番号一括 ~200 行 / フォーム Tab 順 ~475 行**。回転系 (`rotatePageBy` 周辺) は完成領域のため**動かさない**。完了条件 = npm test (済) + 実機一巡 (開く→編集→保存→書き出し→印刷→タブ tearout/dock-back。今回動かした**分割保存パネルは重点確認**: 開閉/分割点クリック/パート名/日付プレフィックス/分割確定/パネル内サムネの D&D・右クリック)。なお renderer.js に exporter の未使用 import 3 件 (`composePageImage`/`composeRegionImage`/`byteCopyEligible`、その1/その2 時点からの残置) — 触るなら別 cleanup commit で。**着手動機 = 下記テキスト表示ずれ (§8.2 🆕) の根治 (案 C) を「次の大物」と位置づけたため**。memory [[project_s6_refactor_progress]]
...
renderer.js は中央 orchestrator として残し、機能別モジュールに分散 (B2 完了時 4,472 行 → 2026-07-10 実測 8,019 行に再肥大 → **S6 リファクタ = REVIEW-2026-07 #8 で 2026-07-23 にその1〜その4 抽出完了、5,646 行**。残り = 実機一巡、冒頭の未リリース欄参照):
...
renderer.js に残る責務: `isOpen` / `projectStore` / `history` / `viewer` 等の core state、`refreshViewer` / `actionOpen` / `openPdfPath` / `actionExport` (別名ダイアログ→save-flow 委譲)、search、回転系 (`rotatePageBy` — 完成領域)、各種メニュー glue。**S6 の抽出は全完了** (残りは実機一巡 + 余力の小物 2 件、冒頭の未リリース欄参照)。
...
**決定 (ユーザー承認 2026-07-23)**: 根治 = **案 C: 非編集時のテキスト表示を wrapCanvasText 採寸の「1 行=1 要素」絶対配置に統一** (v2.0.13 ベクターテキスト層の 1行=1op 資産を流用)。編集中は contentEditable 維持 (IME 制約)。**ユーザーが許容済みの制約**: ①編集終了時に文字がスナップして動くことがある ②編集中は従来のずれが残る (位置判断は編集を抜けてから) ③禁則の見え方が編集中/確定表示で変わる (wrapCanvasText は禁則なし) ④既存文書の画面折返しが変わって見える。**順序 = #8 S6 リファクタ完了 → ADR-0002 の supersede/追補 ADR 起草 (正は mupdf でなく canvas 採寸に変更、mupdf-layout.js は復活させない) → 案 C 実装 → β トライアル**。案 B (確定表示トグル) は C 決定により見送り。**つなぎ運用: 位置の最終確認は印刷プレビュー (exporter 経路 = 真の位置) で行う**。なお今回の誤位置ファイルは ADR-0026「編集に戻す」で復旧可能。memory [[project_text_layout_unification_plan]]
...
- `src/renderer/renderer.js` — 中央 orchestrator (B2 で 4,472 行に圧縮 → 8,019 行に再肥大 → S6 リファクタ = REVIEW-2026-07 #8 で 2026-07-23 に 5,646 行へ)
...
│   └── renderer/
│       ├── index.html
│       ├── renderer.js                   # 5,646 行 (中央 orchestrator、S6 その1〜4 抽出後)
...
- **S6 (split-view + sidebar-thumbs 抽出)**: B2 残置責務。**2026-07-23 に抽出完了 (REVIEW #8)** — その1 image-export / その2 save-flow / その3 sidebar-thumbs / その4 split-view 抽出済、残りは実機一巡 (冒頭の未リリース欄参照)
