...
最終更新: 2026-07-08
現在のバージョン: **v2.0.13-beta.3 (2026-07-07 リリース、β トライアル配信中)**。直前の stable は **v2.0.11 (2026-06-30)**。β.1〜β.150 の業務並走を経て 2026-06-05 に v2.0.0 stable へ卒業 (Win+Mac+Linux 3 OS 配布)。**ADR-0026「戻せる確定保存」(v2.0.12-beta.1) と ADR-0027「ワークスペースの整理」(beta.2) に加え、v2.0.13-beta.1 で「MS 明朝が印刷で薄い」のベクターテキスト埋め込みによる構造解決を投入し、`releaseType=prerelease` のまま β トライアル継続中** (ADR-0027 は実機確認済み、ADR-0026 は業務での往復動作を運用確認、v2.0.12-beta.3=パスワード平文化警告 / beta.4=byte-copy ゲート一本化は実運用の中で確認する扱い → 不具合なければ stable v2.0.13 へ昇格)。**⚠️ stable へ昇格する時は `package.json` `build.publish.releaseType` を `prerelease`→`release`、`version` を `2.0.13-beta.x`→`2.0.13` に戻す** (β タグ `v*-beta.*` は CI で Windows のみビルド、stable タグは 3 OS)。**基本は stable 運用** (重大バグは patch 2.0.x、新機能/大物は ADR 起草後 → β トライアル → 昇格)。**Mac/Linux を Windows 同等 (印刷/FAX/Office/署名) にするには追加開発が要る (§15.6)**。
...
**フェーズ**: **stable 運用中 — 現在 v2.0.11 (2026-06-30)**。β.1〜β.150 → 2026-06-05 に v2.0.0 stable、以降 patch を v2.0.11 まで。**Windows で業務フル運用が実証済** (Mac/Linux は中核は動くが印刷/FAX が Windows 専用実装、§15.6)。**stable 残務は全クローズ済** — クラッシュ診断ロガーは β.149 で撤去済 (`logCrash`/`crashLogPath`/`print-tick` 等は src から消滅、残るは comment 1 件のみ。**§8.2 #3 と末尾の撤去 TODO リストは履歴**=既に適用済)。**現在のオープン項目は §8 の実機確認待ちが中心** (直近 patch の実機検証: v2.0.8 回転のみ上書き保存 / v2.0.9 印刷ダイアログ調整中の Adobe 早期 close / v2.0.10 サムネ削除後の選択位置 / v2.0.11 サムネ並び替えの保存・他アプリ反映)。**調査中の重大候補: A3 横向き PDF の通常印刷が天地さかさま (180°)** — 事務所複合機でテスト依頼中、方針未確定 (§8.2 先頭参照)。**2026-07-06 報告の 3 件のうち ①flate エラー ③MS明朝一覧選択は v2.0.13-beta.2 で修正済 (実機確認待ち)、②一括回転の吹き出し非追従だけ未解決**: ②は再現不能化 (ダメだったのは「先頭に Word 差し込み→下書き保存→同一セッションで一括回転」の 1 回のみ、閉じ直すと追従。回転自体も効かず)。**コード精査 (2026-07-06) での最有力仮説 = `rotatePageBy` 冒頭の無言 return** (`renderer.js:4407` の `viewer._pages.find` 失敗で回転もキャリーも丸ごとスキップ、エラー表示なし=症状一致)。先頭差し込み後の viewer._pages/サムネ DOM/選択 Set の pageNo 不整合疑い。次の一手=無言 return 2 箇所に診断ステータス表示を足す (5 行、Adobe 対策と同じ診断段階強化パターン)。詳細は §8.2「🔴」ブロック。**あわせて同日 (7/6)、K-SystemZ 側から workspaces バックアップ容量調査の照会があり回答済** (デスクトップに調査報告と回答書、要旨と wal/shm 消し残しバグ修正は冒頭 patch 一覧 v2.0.13-beta.2)。**2026-07-07 の新規報告「回転ページの吹き出し本文が印刷/別名保存でだけ枠からはみ出す (画面は正常)」は v2.0.13-beta.3 で同日修正・実機確認済み** — exporter の callout 分岐だけ content rotation を無視 (viewer/exporter 描画二重実装の片側抜け)。🔴(2) とは別問題。冒頭 patch 一覧 v2.0.13-beta.3 / memory [[project_callout_rotation_exporter_fix]]。**実装済・β トライアル中の新機能: 戻せる確定保存 (ADR-0026, v2.0.12-beta.1)** — 下書き/確定の二律背反 (下書き=他アプリで見えない / 確定=後から編集不可) を確定の可逆化で解決。2026-07-01 に ADR 確定・実装 (commit `7b9b822`) → β 配信し、自分の業務で実機の往復動作を運用確認中 (不具合なければ stable へ昇格)。詳細は冒頭の patch 一覧 v2.0.12-beta.1 / §8.2 先頭 / `docs/adr/0026`。**v2.0.13-beta.1 で「MS 明朝が印刷で薄い」をベクターテキスト埋め込みで構造解決** (β31→β141 のラスタ濃度補強の理論上限を実機検証シートで確定→経路切替。Adobe 経路+書き出しのみ、legacy/FAX は従来ラスタ。冒頭 patch 一覧 / memory [[project_vector_text_mincho]])。**2026-07-05 に全体レビューを実施し、対応タスク 11 件 (workspace 膨張 / バックアップ / Electron 41 EOL ほか) を `REVIEW-2026-07-TODO.md` に起票** (§8.2 参照)。**8 件完了 (2026-07-05〜06)** — #10 ゴミログ削除 / #6 依存 exact 固定 / #5 Electron 41 EOL 方針 = (a) 一時容認で決定 (ADR-0004 追記) / #11 PAT 失効 2027-05-11 確認 + リマインド / **#1 workspace お掃除 (ADR-0027 → v2.0.12-beta.2 で実装・実機確認済み)** / **#2 workspaces バックアップ運用開始** (`docs/backup/`) / **#3 パスワード平文化警告 (beta.3)** / **#4 byte-copy ゲート一本化 + 総当たりテスト (beta.4、印刷の末尾ページ削除落ち 2 件を同時修正)**。残りは全て待ち条件付き: #9 (β 所感待ち)・#7 (stable 昇格時)・#8 (次の大物前)。β.1〜β.150 の経緯詳細は `CHANGELOG-history.md` (2026-06-22 に本書から退避) と §6.4 のポインタを参照。
...
#### 🔴 大部 PDF の (1) 別名保存 flate エラー ✅v2.0.13-beta.2 で実装済 (実機確認待ち) + (2) 複数ページ一括回転で吹き出し非追従 (未解決・再現待ち) — 2026-07-06
...
#### 🆕 Mac 版への workspace 資産引き継ぎ — 検討完了 + 可搬性 fallback 2 箇所実装 (2026-07-08、コミット済・未リリース)
...
#### ✅ MS 明朝を「フォント一覧から選択」するとベクター濃度化 (v2.0.13) が効かず薄いまま — 2026-07-06 → **v2.0.13-beta.2 で解消 (実機確認待ち)**

**解消済**: `fonts.js` の `isMsMinchoFontName` (MS明朝系フォント名の正規化、MS P明朝・他明朝系は意図除外) を `vectorTextCandidate` と `_needsHairlineStroke` に適用 — 詳細は冒頭 patch 一覧 v2.0.13-beta.2。実機で「フォント一覧の MS明朝」の印刷濃度が内蔵「明朝」と同等になれば完全クローズ。以下は当時の記録。
