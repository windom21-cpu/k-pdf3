...
最終更新: 2026-05-19
現在のバージョン: **v2.0.0-beta.96** (autoUpdater 経由で配布中)
...
git log --oneline | head -20       # 最新は β96
...
1. **β.96 Adobe 残留拡張 kill の実機検証 → 必要なら exe whitelist 調整** — 次回 FAX 印刷時に Adobe が残ったら crash.log の `pdfreader-cleanup` 内 `adobeRelatedAtCleanup` (実在の Adobe 系プロセス名一覧) を共有してもらう → ユーザ環境固有の exe 名が判明するので、kill 対象を精密化 (現状は `Acro|Adobe Acrobat|AdobeAcrobat|adcef|acrobat` パターンで拾い、`AdobeARM/CollabSync/NotificationClient/IPCBroker/UpdateService` を whitelist 除外)
2. **印刷後 temp PDF が K-PDF3 に再オープンされる副次現象** (β.95 ログで発見) — Adobe 印刷後に temp_print/*.pdf が K-PDF3 に「開け」と渡される。実害は軽微だが余計動作。要調査 (低優先)
...
- ~~D&D「開かない」根因確定 → 修正~~ ✅ **β.90 で根治**。真の機序は「primary window 閉じ + B3 子ウインドウ alive で zombie 化」「second-instance ハンドラが pendingOpens に push するだけで window を作らなかった」。`createMainWindow()` 呼出を追加して自己復旧経路完成
- ~~印刷後 Adobe 残留~~ → β.95-96 で構造解消の試み (実機検証中、上記 #1)
- ~~FAX 経路の縮小事故~~ ✅ **β.92 で Adobe `/p` 経路 + 規定プリンタ一時設定** で根治 (Chromium PDFium の fit-to-paper は API 制御不能のため経路自体を変更)
- ~~FAX 宛先記憶再発~~ ✅ **β.93** で `applyCleanFaxDevmode` を Adobe 経路にも適用 (β.61 移植漏れ修正)
- ~~タブ切替時のしおり混在 + ページジャンプ~~ ✅ **β.94** で `clearBookmarkDom` + scroll 三段復元
...
- 印刷後 Adobe 自動 close (β.95-96 で構造解消の試み、実機検証中)
- 「後で」仮説の恒久対応 (実装)
- 業務並走 1〜2 週間で重大バグなしの確認
- **クラッシュ診断ロガー撤去** (β51 + β75 D&D + β.85 Adobe 残留診断 + β.90 primary-window-closed + β.96 Adobe 拡張検出): `crashLogPath()` / `logCrash()` / `print-route` 等のログ呼び出し / `kpdf3:open-crash-log` IPC / preload `openCrashLog` / index.html の `data-action="open-crash-log"` / `actionOpenCrashLog` / `drop-*` / `gap-drop-file` / `os-open-received` / `j5-zombie-kill-*` / `second-instance-*` / `primary-window-closed` / `kpdf3:log-diag` IPC / `pdfreader-cleanup` の `survivors`/`survivorsExtra`/`killDetails`/`newPidsByExe`/`preExistingPidsByExe`/`adobeRelatedAtCleanup`/`extraKilled` / `pdfreader-process-closed`
