...
最終更新: 2026-05-25
現在のバージョン: **v2.0.0-beta.132** (autoUpdater 経由で配布中、機能凍結ライン)
...
**フェーズ**: **β 卒業準備に移行 (2026-05-25)**。β.131 を機能凍結ラインに、ここから 1 週間 (〜 2026-06-01 目安) は bug fix のみで業務並走。並行で stable 残務 (qpdf Mac/Linux 同梱 / 診断ロガー撤去) を仕込み、重大バグなしを確認したら v2.0.0 stable へ。M6 残のうち annotation proxy / qpdf sanitize / 真の墨消しは β.83-.85 で完了、**「後で」仮説の恒久対応は β.132 で投入** (CancellationToken + autoInstallOnAppQuit=true + ラベル整理)。**CI release 3-OS race も β.132 で構造解消** (Mac/Linux に `needs: build-windows` を付与)。

**直近完了 (β71〜β132、2026-05-14〜25)**:
...
1. **β.131 を機能凍結ラインに** — ここから 1 週間 (〜 2026-06-01 目安) は bug fix のみで業務並走。新機能は投入しない。重大バグなしを確認した上で v2.0.0 stable へ進む
...
3. **クラッシュ診断ロガー一式の撤去** (stable 残務 #6、最後) — β.51-.132 で累積した診断系を撤去。**#2 (qpdf Mac/Linux) と現行 β.131-.132 修正の安定確認が完了してから着手** (= 1 週間並走後)
4. **β.132 autoUpdater 修正の実機検証** — 次回 β.133+ への更新時に: (a)「ダウンロード?」キャンセル ラベル「閉じる」+ 「次回起動時にもう一度お聞きします」表示、(b) ダウンロード中の busy modal に「中止」ボタン → 押したら partial 掃除、(c)「次回起動時に適用」を選んでアプリ終了 → 次回起動で新版が立ち上がる (`autoInstallOnAppQuit=true` の効果)
5. **β.131 Save As タブ表示 + paste aspect の実機検証** — Save As でリネーム保存 → タブラベルが新ファイル名に。クリップボード paste 画像 → 四隅ハンドルで縦横比を保ったまま変形
6. **下敷き印刷の精度キャリブレーション** (β.80 で本体実装済) — 用紙送り誤差で X/Y 数 mm ズレる可能性。実機テストでズレ量を測定 → 必要なら (a) プリンタ別 X/Y オフセット (b) 倍率補正 (c) トレイ別 (d) 申請書別キャリブ を段階追加
7. **印刷後 temp PDF が K-PDF3 に再オープンされる副次現象** (β.95 ログで発見) — 実害は軽微だが余計動作。低優先
8. **β.96 Adobe 残留拡張 kill の実機検証 → 必要なら exe whitelist 調整** — 次回 Adobe 残留が再発したら crash.log の `pdfreader-cleanup` 内 `adobeRelatedAtCleanup` を共有してもらう (現状は `Acro|Adobe Acrobat|AdobeAcrobat|adcef|acrobat` パターンで拾い)
...
- 🔴 **クラッシュ診断ロガー撤去** (stable 残務 #6、最後): `crashLogPath()` / `logCrash()` / `kpdf3:log-diag` IPC / preload `openCrashLog` / index.html の `data-action="open-crash-log"` / `actionOpenCrashLog` / `drop-*` / `gap-drop-file` / `os-open-received` / `j5-zombie-kill-*` / `second-instance-*` / `primary-window-closed` / `pdfreader-cleanup` の `survivors`/`survivorsExtra`/`killDetails`/`newPidsByExe`/`preExistingPidsByExe`/`adobeRelatedAtCleanup`/`extraKilled` / `pdfreader-process-closed` / `pdfreader-jobs-drained` / `print-cancel-by-user` / `pdfreader-followup-snapshot` / `font-fallback-callback`。**他残務の安定確認後に適用**
- 🟡 **業務並走 1 週間で重大バグなしの確認** (2026-05-25〜06-01 目安)
