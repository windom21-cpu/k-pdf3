# K-PDF3 開発履歴アーカイブ (CHANGELOG-history.md)

> 2026-06-22 に `HANDOVER.md` から肥大化していた歴史的記録 (完了 β の詳細変更ログ・
> §6.4 β 全表・各 patch の full 詳細・§8 の完了済み項目) を退避したもの。
> **現役の設計判断・禁止事項・次にやることは `HANDOVER.md` を正とする。** 本ファイルは
> 「いつ何を・なぜそうしたか」の経緯を grep するための保管庫。製品は stable v2.0.7。

---

## A. 直近パッチ (v2.0.1〜v2.0.7) の full 詳細

現在のバージョン: **v2.0.7 stable (2026-06-22 リリース)**。stable 後の patch 7 本: **v2.0.7 = ページ回転 (userRotation) のみを施したページが、別名/上書き PDF 保存と全ページ印刷で回転が他ビューア/紙に反映されない不具合を修正 — 回転は元 PDF バイトに焼かず `pages.user_rotation` に持つ viewer 専用変換で、書き出し/印刷の「未編集なら元バイトを byte-copy」判定 (`isCopy`) が overlay の有無しか見ず userRotation を無視していたため、回転だけのページが元のまま byte-copy され回転が消えていた (K-PDF3 で開き直すと viewer が userRotation を再適用するので回って見え、発見が遅れた)。`isCopy` 判定 3 か所に「回転ありページがあれば byte-copy せず再合成経路へ」を追加: `renderer.js` `actionExportToPath` (保存/別名、4124 付近)、`print-flow.js` `actionPrintViaReader` (Adobe あり=実機印刷経路、473)、同 legacy 印刷 (Adobe 無し環境、603)。実機印刷は `actionPrintViaReader` に分岐するため legacy 側だけでは効かず両方修正。再合成経路は回転ページのみ `assembleHybridPdf`→`_placeRotatedSourcePage` でベクター維持のまま回転をベイク (β.142 と同経路・`effRot = sourceRotation + userRotation` を CW でベイク)、非回転ページは従来どおり verbatim copyPages なので画質/サイズ/セキュア書き出しに無影響。画像書き出し (β.97) ・範囲/単ページ書き出し (always-rasterized) ・FAX/下敷き印刷 (再合成経路) は元から正常で無関係。実機 A4 縦サンプル (`Desktop/rotate-test-A4.pdf` を生成) で #2 別名保存・#4 全ページ印刷の回転落ちを再現→修正確認、`rotate-place.js` + mupdf で 90° ベイク (矢印 上→右) を headless 実証、node テスト 9 種全 pass。β.142 は overlay 付き回転ソースの天地さかさま修正、本件は overlay 無し・回転のみの byte-copy 漏れで別バグ。詳細 memory `[[project_rotation_only_bytecopy_bug]]`** / **v2.0.6 = 下敷き印刷の「紙全体が上下さかさま」事故への対策 — 真因は K-PDF3 ではなく Adobe 印刷ダイアログの「向き=自動」が“ほぼ白紙の下敷きページ”の天地を判断できず180°回すこと (実機テストで確定。K-PDF3 の overlay 配置座標は通常印刷と完全一致、overlay-only 出力の組み立ては β.80 から無変更 ⇒ Adobe 既定挙動の変化で顕在化、ユーザーの「当初は正常だった」とも整合。β.142 は背景込み経路しか直しておらず overlay-only には未到達だった)。K-PDF3 側から確実に制御できないことを実地で確認 (✗白で隠したフォーム埋込=Adobe は不可視内容を向き判定に使わない / ✗DEVMODE 向き強制=トレイ/両面/カラーの DEVMODE 押込は silent fallback 専用で Adobe `/p` 経路は不使用・案M/N/ζ がドライバ制約で全滅→Adobe に逃がした壁と同じ / ✗180°先回り=Adobe 仕様変更でサイレント故障し法律文書に危険)。よって自動補正はせず、**下敷き印刷の確認ダイアログで「向き=縦」を赤枠警告＋必須チェック (ON まで [印刷] 無効・storageKey なしで毎回再確認) で人に意識的に設定させる運用**に。`dialogs.js` `customConfirm` に `warning`(赤枠) + `checkbox.required` を汎用追加 (index.html/style.css/print-flow.js)。renderer のみ変更・全テスト pass・実機UI確認はビルド後。詳細 memory `[[project_underlay_print_180flip_adobe]]`。**⚠️ Adobe で毎回「向き=縦」を選ぶ運用が前提 (Adobe は縦設定を記憶しないことがある)** / **v2.0.5 = 更新後 PDF 復元の実地検証用テストリリース (コード変更なし = v2.0.4 の復元ロジックと完全同一、version だけ繰り上げてテストの妥当性を担保)。検証手順: ① 検証機に v2.0.4 を入れる (autoUpdater は latest=v2.0.5 を直接出すので、必ず先に v2.0.4 を手動インストールして「保存側を持つ修正版」に乗せる。v2.0.3 のまま v2.0.5 へ飛ぶと v2.0.3 の wipe バグで復元されない) → ② PDF を 2-3 枚開く → ③ v2.0.5 更新を適用して再起動 → ④ 開いていた PDF が復活すれば OK** / **v2.0.4 = v2.0.3 の「更新後 PDF 復元」のバグ修正 — 最後のウィンドウを閉じる (= 更新で終了する) 瞬間に後始末ハンドラ (webContents destroyed) が `session.json` を空 `[]` で上書きしていた根因 (v2.0.3 で混入) を、「他ウィンドウが生きている時だけ trimmed union を再保存、最後の 1 枚なら据え置き」(`shouldRepersistOnWindowClose`) に修正。dev 実機で write→quit 保持→version 変化で自動復元の全経路を確認 (`session.json` が `{2.0.3,[sample.pdf]}` → 2.0.4 起動で sample 再オープン → `{2.0.4,[sample.pdf]}`)。⚠️ **構造的制約**: 復元は「保存側コードを持つ版から更新した時」のみ機能する — (a) v2.0.2 は保存側が無いため v2.0.2→v2.0.3 は復元不可、(b) v2.0.3 は本バグ持ちで quit 時に session を空に潰すため v2.0.3→v2.0.4 も復元されない。実働するのは v2.0.4 以降の更新 (v2.0.4→次) から。`test/session-store.test.mjs` 16 pass** / **v2.0.3 = 分割ビュー(分割保存)の白紙ページ挿入を k-bunkatu 風に刷新 — 入力テキストを中央寄せ＋枠に収まる自動縮小 (72→20pt)＋薄グレー地で描画 (`renderSyntheticPagePixels` 1 関数で画面/サムネ/印刷/書き出しに一括反映、空欄は従来通り純白)。さらに (a) 分割ビューの「＋」挿入区切りを常時表示の青丸バッジで発見容易化し、先頭「＋」をゼロ幅で左ガターに収めて page-1 サムネを 2 行目以降と完全整列 (実測 firstLeft 全行一致)、(b) 左サイドバーで file ドラッグ中に挿入ギャップが伸びて末尾「＋」がウインドウ下に見切れる問題を `height:28px`→負マージン相殺のレイアウト不変方式で解消 (高さ増分 +260px→0)、(c) 自動更新の再起動で更新前に開いていた PDF を復元 (`src/main/session-store.js` に version+openFiles を記録、version 変化時のみ既存 pendingOpens 経路で再オープン、編集内容は fingerprint ワークスペース再利用で保持。`test/session-store.test.mjs` 13 pass、dev 実機で session.json 生成確認)。参考にした別アプリ = `X:\K-system\ksystemz\k-bunkatu` (Python/Flask の PDF 分割ツール、白紙+CIDフォントで目印ページを焼き込む)** / **v2.0.1 = パスワード保護 PDF を開けるように (qpdf `--decrypt` + 98 風入力モーダル、復号版をワークスペースに保存=以降は閲覧/編集/書き出し/印刷が不変)** / **v2.0.2 = その白紙バグ修正 (旧版で暗号化のまま取り込んだ既存ワークスペースが fingerprint 再利用でゲートを素通りし白紙になる問題を、検出を `/Encrypt` 全般に拡張 + 空パスワード先行試行 + 既存ワークスペース自己修復で解消)**。実機でパスワード入力→復号→表示まで確認済 (2026-06-08)。詳細は §15.4 / git log (`feat(open)` / `fix(open)`)。

---

## B. 現状サマリにあった β フェーズ履歴段落

**フェーズ**: **stable 運用中 — 現在 v2.0.5 (2026-06-13)**。β.1〜β.150 の業務並走を経て 2026-06-05 に v2.0.0 stable タグを切り、Win+Mac+Linux の 3 OS で正式配布 (autoUpdater は `allowPrerelease=true` なので β.150 テスターも受信)。**stable 後の patch**: v2.0.1=パスワード保護 PDF 対応 (新機能、ユーザー要望)、v2.0.2=その白紙バグ修正 (詳細 §15.4)。**今後も stable 運用**: 重大バグは patch (2.0.x) で対応、新機能/大物は ADR を起こしてから。**⚠️ β を切る場合のみ `package.json` `build.publish.releaseType` を `release`→`prerelease` に戻す** (現状 stable patch は `release` のまま 3 OS フルビルド)。**Mac/Linux を Windows 同等にする (印刷/FAX/Office/署名) には相応の追加開発が要る — 必要作業とハードルの分析を §15.6 に記録 (2026-06-06、ユーザー後学メモ依頼)**。以下は β 期間 (2026-05-10〜06-05) の経緯: β.131 機能凍結ライン以降、業務並走で見つかった重大バグを β.133〜β.146 で対応 (重大バグ修正が主、β.144/.146 は業務並走で明示要望のあった軽微 UX 改善)。stable 残務 (qpdf Mac/Linux 同梱 / 診断ロガー撤去) を並行で仕込み、重大バグなしを確認して v2.0.0 stable へ移行。**qpdf Mac は 2026-06-05 に M1 実機検証 PASS で実質クローズ、stable まで残るゲートは診断ロガー撤去 (#6) のみ**。M6 残のうち annotation proxy / qpdf sanitize / 真の墨消しは β.83-.85 で完了、「後で」仮説恒久対応は β.132 で投入、**β.134 で巨大 PDF (712MB 級裁判所謄写) を SQLite BLOB ではなくサイドカーファイル化して開けるように構造修正**、**β.136 で墨消し書き出しの透過 PDF 黒背景バグを修正**、**β.138 で印刷送信中モーダル消失失敗 (Adobe Pro DC 親子分離下の Path B 沈黙) を構造解消**、**β.139 で installer の portable target 撤去 + 関連付け書込を sentinel 付き customInstall macro で初回 install 時のみに限定 (autoUpdater 更新時の「規定アプリリセット」通知 + 「アイコンあり/なし K-PDF3 が 2 つ表示」現象を構造解消)**、**β.140 で 2026-05-27 業務並走フィードバック 9 件 (テキスト UX 6 + 保存 UX 3) + MS 明朝が印刷物で薄くドット化する根本対策 (hairline 経路に fillText 二重描きで AA 縁の濃度のみ上げる、glyph 太さは不変) を投入**、**β.141 で β.140 の追い込み 2 件 — (a) 明朝印刷密度を 2 回打ち → 4 回打ち (AA α 0.75 → 0.9375) に強化して横線のドット感も解消、(b) 配置済みテキスト枠を選択しても options bar が表示されない (β.140 で `refreshModeOptionsBar` に text 選択時の分岐を入れ忘れた配線漏れ) を修正**、**β.142 で回転した元 PDF (/Rotate≠0) の下敷き/通常印刷でオーバーレイ (記入値) だけが天地さかさまに印刷される重大事故を構造解消 (申請書下敷き印刷で実害発生、β4/β5 以来の潜在バグ — `assembleHybridPdf` が userRotation のみ補正しソース /Rotate を無視 + pdf-lib CCW vs PDF/mupdf CW の方向取り違え)**、**β.143 で吹き出しの後付け書式変更 (text 同様にフォント/サイズ/色/太字を選択後に変更) + 吹き出し枠はみ出し (入力画面では収まるのにサムネ/印刷で本文が枠外に漏れる) を構造解消 (採寸と exporter の折返し幅の下限不一致が根因、「枠を本文に合わせて自動拡大」方式で対応)**、**β.144 で業務並走の微調整要望 2 件 — (1) マーカーに「直線」種類を追加 (従来の範囲ドラッグに加え、一定の太さで水平にまっすぐ引く蛍光ペン式。種類/色/太さを options bar で選択・永続化)、(2) テキスト配置を sticky 化して連続入力可能に (1 枠ごとにモードが解除されていたのを維持。編集確定クリックでの二重配置は handlePagePointerDown の編集中ガードで抑止)**、**β.145 で β.144 以降に main へ積んだインフラ/セキュリティ整備を配布 (新機能なし) — Electron 38(EOL)→41 化 (better-sqlite3 12.10.0 で 41 prebuild) + hardening (全 webContents で window.open/リモート遷移拒否 + CSP) + qpdf Linux 同梱 + CI を Node24 対応 actions SHA 固定 + 別窓の ESM `require` 回帰修正。Electron 41 の Windows 実機配布は β.145 が初**、**β.146 でツールバーをアイコン化 (◎ アイコンのみ / ○ アイコン+小ラベル / △ は » 収納) + 全ボタンにホバー用途解説 + 幅あふれ時に表示倍率→検索→回転→… の順で実コントロールを「»」へ ResizeObserver で動的退避 (項目を「縮めない」方式にしたので高さ一定・テキスト縦並び/アイコンずれが起きない)**、**β.147 で β.134 巨大 PDF サイドカー (`.kpdf3.source.pdf`) の orphan 掃除を追加 (stable 残務 #7)**。**stable 残務の qpdf Mac は arm64 専用ビルドに確定 (2026-06-05、`package.json` arch=`["arm64"]`) し、同日 事務所 M1 で自己完結バンドルを `vendor/qpdf/mac/{bin,lib}/` に同梱完了 (commit `5a52bbb`、universal2/Rosetta/lipo 不要)。これで qpdf は 3 OS とも同梱済。**2026-06-05 に事務所 M1 (macOS 26.5) でローカルビルド検証を実施し、同梱 qpdf 配置/単体起動/依存ゼロ・GUI セキュア書き出しでのメタ除去・quarantine 付き (配布相当) での子プロセス qpdf 起動を全 PASS (`MAC-VERIFY-RESULT.md`)。これで stable 残務 #5 は実機確認まで完了**。検証中に発見した **byte-copy 経路 (未編集 PDF を「名前を付けて保存」セキュア ON) で qpdf を迂回しメタが残る全 OS 共通バグを同日修正** (`copy-source-pdf` ハンドラに secureExport を追加、ベクター維持で Info/XMP 除去、commit `1927e64`、未タグ)。

---

## C. 直近完了 β71〜β147 詳細変更ログ + β 卒業ロードマップ
...
- **β146** (2026-06-04): **ツールバーのアイコン化 + 幅あふれの「»」動的退避** — 業務並走フィードバック「小さい/低解像度モニターだとツールバーのボタンが常に折り返し、文字が縦並びになって不格好・使いにくい。大きい高解像度画面なら問題ない」への対応。
  ...
  - 注: DOM/レイアウト依存の renderer 機能のため node 自動テスト外 (手動確認のみ方針)。`node --check` pass、`npm test` 既存スイートは無改修で全 pass。**実 CSS + 同一退避アルゴリズムの忠実プレビューを headless Chrome で 1280/820/620px の 3 幅レンダリングし、高さ一定・表示→検索→… の順退避・アイコン無ズレを確認済**。(β.146 はアップデート配信でユーザー実機確認済・OK)
- **β147** (2026-06-05): **β.134 巨大 PDF サイドカー (`.kpdf3.source.pdf`) の orphan 掃除 (stable 残務 #7)** — β.134 で 200MB 超のソース PDF は SQLite BLOB を避け workspace 隣の `{id}.kpdf3.source.pdf` に退避するが、上書き保存 (`openWorkspace({force:true})`) で `.kpdf3` を潰した際に `.source.pdf` だけが取り残されて orphan 化していた (`getSourceBytes` は workspace DB の `external_path` 行経由でしか到達しないため、`.kpdf3` が無ければ二度と読めず disk を食うだけ。アプリは registry 行を DELETE せず workspace を一切削除しないので、orphan 発生源は force 上書き 1 箇所のみ)。
  - **(1) 発生源封鎖**: `sqlite-store.js` の force 経路で `-wal`/`-shm` と並べて `filePath + ".source.pdf"` も `rmSync({force:true})` (無ければ no-op)。
  - **(2) 起動時回収**: Electron 非依存・dir 引数注入の新モジュール `src/main/sidecar-sweep.js` を新設 (`findOrphanSourceSidecars` 純粋判定 + `sweepOrphanSourceSidecars(dir)`)。`main.js` の `whenReady` で `setImmediate` 内 fire-and-forget。**削除対象は「兄弟 `.kpdf3` が無い `*.kpdf3.source.pdf`」だけ**で `.kpdf3` 本体・無関係ファイルには一切触れない。**空 `.kpdf3` ワークスペースの掃除は意図的にスコープ外** (overlay 作業を持つため保持ポリシーが要る)。
  - `workspace-registry` が `electron` を import するため、当初そこに置いた純粋判定が plain Node でテストできず、独立モジュールに切り出した経緯あり。`test/sidecar-sweep.test.mjs` (純粋判定 6 + 実 temp dir IO 検証 9 = 15 pass) を追加し `test` チェーンに組込み。全スイート 415 pass / 0 fail。
...
24. ✅ **qpdf Mac/Linux バンドル** — **stable 配布対象に Mac/Linux も含めることが 2026-05-25 に確定** (ユーザー判断)。**2026-06-03 に Linux 完了** + **2026-06-05 に Mac 完了 (commit `5a52bbb`)** で 3 OS とも同梱済。Linux: 公式 portable v12.3.2 を `vendor/qpdf/linux/` に bin+lib 同梱。Mac: 事務所 M1 実機で Homebrew qpdf 12.3.2 を `install_name_tool` 手動バンドル (arm64 専用、`@executable_path/../lib` + ad-hoc 署名) して `vendor/qpdf/mac/{bin,lib}/` に配置。`qpdf-sanitize.js findQpdfBinary` は win=flat / mac・linux=bin+lib のレイアウト差対応済、`package.json` の mac/linux `extraResources` 設定済。**2026-06-05 に M1 実機ローカルビルド検証 PASS (`extraResources` コピー + GUI セキュア書き出しのメタ除去 + quarantine 下の子プロセス qpdf 起動)。残務 #5 実機確認まで完了**
...
31. ~~巨大 PDF (200MB 超) で workspace 作成失敗~~ ✅ **β.134 で構造解消** (サイドカーファイル + `external_path` カラム)。実機 712MB 謄写 PDF で 2.4 秒 open 確認済。**継続観察**: ~~orphan サイドカー残留~~ ✅ **β.147 で orphan `.source.pdf` の startup スキャン掃除を実装** (発生源封鎖 + 起動時回収、`sidecar-sweep.js`)。**空 .kpdf3 累積 (β.132〜β.133 失敗試行残り 9 件等) は引き続き未掃除** — overlay 作業を持ちうるため保持ポリシー要、業務影響なし・低優先で stable 前に検討余地
...
| **β146** | 06-04 | **ツールバー アイコン化 + 幅あふれの「»」動的退避** — 業務並走 (小さい/低解像度モニターで折り返し→不格好) 対応。(1) 22 ボタンを ◎ アイコンのみ / ○ アイコン+小ラベル / △ は » 収納に整理、16px モノクロ SVG (`.ti`)、全ボタンにホバー用途解説 `title`。(2) 項目を `flex:0 0 auto`+`nowrap` で縮めない設定にし、`ResizeObserver`+rAF で 表示倍率→検索→回転→… の順に実要素を `#overflow-dynamic` へ退避 (高さ一定・アイコン無ズレ)。menu-bar.js 非依存、検索 margin-left:auto 撤去。手動確認のみ、`node --check` pass / `npm test` 既存全 pass / headless 3 幅プレビュー確認済。アップデート配信でユーザー実機確認済・OK |
| **β147** | 06-05 | **orphan `.source.pdf` サイドカー掃除 (stable 残務 #7)** — β.134 サイドカーが上書き保存で取り残される orphan を 2 層で構造的に閉じる。(1) `sqlite-store.js` force 経路で `.source.pdf` も道連れ削除 (発生源封鎖)。(2) `src/main/sidecar-sweep.js` 新設 (Electron 非依存・dir 注入)、`whenReady` で「兄弟 `.kpdf3` 無しの `*.kpdf3.source.pdf`」のみ best-effort 削除。`.kpdf3` 本体・空ワークスペースには触れない (空掃除は保持ポリシー要のためスコープ外)。`test/sidecar-sweep.test.mjs` 15 pass (純粋判定6+実 temp dir IO 9)、全 415 pass。CI 通過・配布済 |
...
## F. §8「stable に向けた残作業」のうち機能実装 (✅ 完了済) 全リスト
