...
import {
  ...
  setTabOrderEditMode,
  isTabOrderEditMode,
  updateTabOrderListActive,
  refreshTabOrderListPopupIfOpen,
} from "./form-tab-order.js";
import {
  createTabState,
  initTabManager,
  getActiveTab,
...
// β.82 (B-6): 「Tab 順を編集」モード (tabOrderEditMode) は
// form-tab-order.js が state ごと所有 (§4.4 パターン 3)。
// 読みは isTabOrderEditMode()、切替は setTabOrderEditMode()。
...
// β.82 (B-6): Tab 順編集モード (form-tab-order)
initFormTabOrder({
  projectStore: () => projectStore,
  history: () => history,
...
function setFormFillMode(on) {
  ...
  if (formFillMode) {
    ...
    if (isTabOrderEditMode()) setTabOrderEditMode(false);
...
// β.82 (B-6): Tab 順編集モードの ON/OFF トグルボタンの配線は
// form-tab-order.js 側に移動 (S6 その6)。
