...
export async function actionPrintOverlayOnly() {
  ...
  //    そのまま改行表示される。
  // ⚠ 下敷き印刷は「ほぼ白紙のページ」を刷るため、Adobe の「向き=自動」が
  //   天地を誤判定して 180° 上下逆さまに印刷する事故が起きる (実機確認済)。
  //   原因は Adobe 側 (可視内容でしか向きを判定しない) で K-PDF3 からは
  //   確実に制御できないため、毎回はっきり警告し、ユーザーに「向き=縦」を
  //   意識的に設定・確認させる運用とする (自動回転トグル等のサイレント
  //   補正は Adobe 仕様変更でサイレント故障するため不採用)。
  const proceed = await customConfirm({
    title: "下敷き印刷",
    warning:
      "⚠ 重要 ⚠\n" +
      "Adobe の「向き」を必ず【縦】にしてください。\n" +
      "「自動」のままだと記入内容が上下さかさまに印刷されます。",
    message:
    ...
    okLabel: "印刷",
    cancelLabel: "キャンセル",
    // 安全ゲート: 毎回チェックを外した状態で表示し、ユーザーが「縦にした」
    // を確認するまで [印刷] を押せない (storageKey なし=記憶しない=毎回再確認)。
    checkbox: {
      label: "Adobe の「向き」を【縦】に設定しました",
      required: true,
    },
  });
  if (!proceed.ok) {
