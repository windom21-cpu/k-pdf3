...
## 更新 (2026-07-05): Electron 41 EOL (2026-08-25) 後の方針決定

### 背景
- Electron 41 のサポート終了が **2026-08-25** に迫るが、42 は better-sqlite3 の prebuild 未対応が継続（PR #1470 取り下げ、issue #1474/#1475 オープン、2026-07-05 時点）。
- 期限までに 42 が通らなかった場合に慌てて悪手（自前ビルド強行等）を打たないよう、事前に方針を決定（2026-07 全体レビュー #5、ユーザー承認済み）。

### 決定: (a) EOL 一時容認 + 監視継続
- 2026-08-25 までに better-sqlite3 の 42 対応が出ない場合、**Electron 41 のまま EOL を一時容認**する。
- 根拠: ローカル完結アプリでリモートコンテンツを開かない、CSP + window.open/遷移拒否の hardening 済（β.145）、攻撃面が狭い。Electron 38 EOL 時に約 3 ヶ月容認した前例あり。
