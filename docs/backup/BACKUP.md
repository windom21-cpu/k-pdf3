...
- **PowerShell から実行する場合**は `/TR` の引用符を `'"C:\...\backup-workspaces.bat"'`
  (シングルクォートで包む) にする (cmd 用の `\"` エスケープはエラーになる)。
- K-PDF3 を開いたまま夜を越すとその晩はスキップされる (`[SKIP]` がログに残らず
  終了コード 1)。翌晩に閉じていれば追いつく。
- **02:00 に PC が起動していないことが多い場合**は、未実行分を次回電源 ON 時に
  追いつき実行する設定を追加する (登録済み、2026-07-05):

```powershell
Set-ScheduledTask -TaskName "K-PDF3 backup" -Settings (New-ScheduledTaskSettingsSet -StartWhenAvailable -AllowStartIfOnBatteries -DontStopIfGoingOnBatteries)
```

  → 実質「朝の電源 ON 直後に 1 回」走る (K-PDF3 起動前なのでガードにも
  かからない)。結果表示のウインドウが 60 秒出るが、何かキーを押せば閉じる。
